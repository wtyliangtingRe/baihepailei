...
function styleLabel(style?: string) {
  if (style === 'black-banner') return '旧 Wiki 模板'
...
function CalloutImage({ image, title }: { image?: ContentCalloutImage; title?: string }) {
  ...
  return (
    <figure className="content-callout-image">
      <img alt={image.alt || title || image.filename || '图文提示图片'} src={image.url} />
    </figure>
  )
}

export default function ContentCallout({ callout }: { callout: ContentCalloutItem }) {
  const hasBody = callout.title || callout.text || callout.quote || callout.sourceUrl
  if (!hasBody && !callout.image?.url) return null

  return (
    <aside className={styleClass(callout.style)}>
      <CalloutImage image={callout.image} title={callout.title} />
      <div className="content-callout-body">
        ...
        {callout.sourceUrl ? (
          <a href={callout.sourceUrl} rel="noreferrer" target="_blank">
            {callout.sourceLabel || '查看来源'}
          </a>
        ) : null}
      </div>
    </aside>
