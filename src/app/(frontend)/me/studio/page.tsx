...
function filtersFrom(params: Record<string, string | string[] | undefined>) {
  ...
  return {
    ...
    perPage: [20, 50, 100].includes(positiveInteger(first(params.perPage), 50))
      ? positiveInteger(first(params.perPage), 50)
      : 50,
  }
}

function studioHref(filters: ReturnType<typeof filtersFrom>, overrides: Partial<ReturnType<typeof filtersFrom>> = {}) {
...
function whereFor(filters: ReturnType<typeof filtersFrom>): Where {
  const and: Where[] = []
  if (filters.q) {
    const or: Where[] = [
  ...
  if (filters.status === 'active') and.push({ status: { not_equals: 'archived' } })
...
async function hideWorkAction(formData: FormData) {
  ...
  const returnTo = text(formData.get('returnTo'), 1000) || '/me/studio'
  if (!id || reason.length < 4 || confirmation !== '隐藏') {
    redirect(`${returnTo}${returnTo.includes('?') ? '&' : '?'}studioError=hide_confirmation`)
  }

  const current = await payload.findByID({ collection: 'works', id, depth: 0, draft: true, overrideAccess: true }) as unknown as StudioWork
  ...
  redirect(`${returnTo}${returnTo.includes('?') ? '&' : '?'}hidden=${encodeURIComponent(id)}`)
}
...
async function restoreWorkAction(formData: FormData) {
  ...
  const id = text(formData.get('id'), 40)
  const returnTo = text(formData.get('returnTo'), 1000) || '/me/studio?status=archived'
  ...
  revalidatePath('/me/studio')
  redirect(`${returnTo}${returnTo.includes('?') ? '&' : '?'}restored=${encodeURIComponent(id)}`)
}
...
export default async function ContentStudioPage({ searchParams }: { searchParams: PageSearchParams }) {
      ...
      <main className="page review-workbench studio-page">
        <section className="review-hero">
          <div className="review-hero-copy">
            <p className="eyebrow">资料库共建</p>
            <h1>提交新作品申请</h1>
            <p className="muted">普通用户不会直接修改正式资料库。你可以提交作品名称、别名、来源和收录理由，之后由编辑审核并建立草稿。</p>
          </div>
        </section>
        <section className="detail-card">
          <h2>发现资料库里没有的作品？</h2>
          <p>新建申请会进入“用户反馈审核”，不会直接公开，也不会覆盖现有条目。</p>
          <Link className="review-button review-button-primary" href="/feedback?type=new_work">新建作品申请</Link>
        </section>
      </main>
    )
  }
  ...
  const [result, activeCount, archivedCount] = await Promise.all([
    ...
    payload.find({ collection: 'works', depth: 0, draft: true, limit: 1, pagination: true, overrideAccess: true, where: { status: { not_equals: 'archived' } } }),
    payload.find({ collection: 'works', depth: 0, draft: true, limit: 1, pagination: true, overrideAccess: true, where: { status: { equals: 'archived' } } }),
  ])
    ...
    <main className="page review-workbench studio-page">
      <section className="review-hero">
        <div className="review-hero-copy">
          ...
          <h1>真正编辑网站内容</h1>
          <p className="muted">这里直接读取 Payload 当前数据库，不是 AI 审核队列。工作人员可以搜索、创建、编辑、隐藏和恢复作品；所有写入保留版本历史，不执行 PostgreSQL 直写。</p>
          <div className="review-safety-note">“隐藏作品”是可恢复的软删除：作品会归档并关闭公开可见性，不会永久删除记录。跨语言合并仍只开放给最高领袖和管理员，并走单组确认。</div>
        </div>
        <div className="review-stat-grid">
          <div className="review-stat"><span>可编辑作品</span><strong>{activeCount.totalDocs.toLocaleString('zh-CN')}</strong></div>
          <div className="review-stat"><span>回收站</span><strong>{archivedCount.totalDocs.toLocaleString('zh-CN')}</strong></div>
        </div>
      </section>

      {studioError ? <div className="review-action-message review-action-message-error" role="alert">隐藏作品需要填写至少 4 个字的理由，并在确认框输入“隐藏”。</div> : null}
      ...
      <div className="review-row-actions">
        <Link className="review-button review-button-primary" href="/me/studio/works/new">工作人员新建作品草稿</Link>
        <Link className="review-link" href="/me/review/content">AI / 内容审核台</Link>
        <Link className="review-link" href="/me/review/feedback">用户反馈审核</Link>
      </div>
      ...
      <nav className="review-queue-tabs" aria-label="内容管理范围">
        ...
        <Link aria-current={filters.status === 'all' ? 'page' : undefined} href={studioHref(filters, { status: 'all', page: 1 })}><span>全部记录</span><strong>{(activeCount.totalDocs + archivedCount.totalDocs).toLocaleString('zh-CN')}</strong></Link>
      </nav>
          ...
          return (
            <article className={`review-row review-content-row${archived ? ' review-merged-row' : ''}`} key={work.id}>
              <header className="review-row-header">
                <div className="review-row-title"><h2>{work.title || `作品 #${work.id}`}</h2><small>作品 ID：{work.id} · 最近更新：{formatDate(work.updatedAt)}</small></div>
                <div className="review-chip-list"><span>{label(work.rank)}级</span><span>{work.reviewStatus || 'pending'}</span><span>{work.status || 'draft'}</span></div>
              </header>
              ...
              <div className="review-content-actions">
                {!archived ? <Link className="review-button review-button-primary" href={`/me/studio/works/${work.id}?returnTo=${encodeURIComponent(currentHref)}`}>编辑完整条目</Link> : null}
                <Link className="review-link" href={canonicalContentUrl('works', work.id)}>查看前台</Link>
              </div>
              {!archived ? (
                <details className="review-safety-note">
                  <summary>隐藏 / 移入回收站</summary>
                  <form action={hideWorkAction} className="review-content-form">
                    <input name="id" type="hidden" value={String(work.id)} />
                    <input name="returnTo" type="hidden" value={currentHref} />
                    <label className="review-content-note"><span>隐藏理由</span><textarea maxLength={800} name="reason" placeholder="例如：重复条目、误建条目、版权或身份信息待核查。" required /></label>
                    <label><span>确认</span><input name="confirmation" placeholder="输入：隐藏" required /></label>
                    <button className="review-button review-button-danger" type="submit">移入回收站</button>
                  </form>
                </details>
              ) : (
                <form action={restoreWorkAction} className="review-content-actions">
                  <input name="id" type="hidden" value={String(work.id)} />
                  <input name="returnTo" type="hidden" value={currentHref} />
                  <button className="review-button review-button-primary" type="submit">恢复为待复核草稿</button>
                </form>
              )}
            </article>
          )
        })}
        {docs.length === 0 ? <section className="review-empty"><h2>没有匹配作品</h2><p>可以换一个译名、数据库 ID，或切换可编辑作品与回收站。</p></section> : null}
      </section>
...
function Pagination({ filters, currentPage, totalPages }: { filters: ReturnType<typeof filtersFrom>; currentPage: number; totalPages: number }) {
  if (totalPages <= 1) return null
  return (
    <nav className="review-pagination" aria-label="内容管理分页">
      {currentPage > 1 ? <Link href={studioHref(filters, { page: currentPage - 1 })}>上一页</Link> : null}
      <span aria-current="page">第 {currentPage} / {totalPages} 页</span>
      {currentPage < totalPages ? <Link href={studioHref(filters, { page: currentPage + 1 })}>下一页</Link> : null}
    </nav>
  )
}
