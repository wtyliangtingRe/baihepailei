...
export default async function EvidenceDetailPage({ params }: Args) {
  const { slug } = await params
  const decodedSlug = decodeURIComponent(slug)

  const detailItem = findDetailItem('evidence' as never, decodedSlug)
  if (detailItem) return <DetailIndexDetail item={detailItem} />

  const index = readSearchIndex()
  if (!index) return <MissingSearchIndex />

  const item = findSearchItem('evidence', decodedSlug)
  if (!item) notFound()
