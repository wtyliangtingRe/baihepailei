...
export function isPublicSearchItem(item: { collection?: string; title?: string; status?: string; catalogStatus?: string }) {
  if (item.collection === 'works' && (item.catalogStatus === 'archived' || item.status === 'archived')) return false
