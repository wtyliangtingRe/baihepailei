...
export async function GET(request: NextRequest) {
  const enabled = normalized(process.env.RADAR_PUBLIC_RELEASE_LAB_MODE).toLowerCase() === 'true'
  if (!enabled) {
    return NextResponse.json({ error: 'not_found' }, { status: 404 })
  }

  const expectedNonce = normalized(process.env.RADAR_PUBLIC_RELEASE_LAB_NONCE)
  const suppliedNonce = normalized(request.headers.get('x-radar-public-release-lab-nonce'))
  if (!expectedNonce || suppliedNonce !== expectedNonce) {
    return NextResponse.json({ error: 'invalid_lab_nonce' }, { status: 403 })
  }

  const expectedDatabase = normalized(process.env.RADAR_PUBLIC_RELEASE_LAB_DATABASE)
  ...
  return NextResponse.json({
    schemaVersion: 'radar-public-release-lab-marker-v01',
    isolatedLab: true,
