...
test('legacy migration entry point fails closed and points to the verified read-only audit wrapper', () => {
  assert.match(migrationWrapper, /migration generation is paused/u)
  assert.match(migrationWrapper, /run-and-package-work-normalization-audit-v01\.ps1/u)
  assert.match(migrationWrapper, /UTF-8\/Base64 v02 audit transport/u)
...
test('normalization candidate audit is database-read-only and produces validated review artifacts', () => {
  ...
  assert.match(normalizationAudit, /validation\.json/u)
