...
test('migration preparer creates an empty current-schema baseline before the additive Radar diff', () => {
  ...
  assert.match(migrationPreparer, /Get-GeneratedPair/u)
  assert.match(migrationPreparer, /\$baseline\.Snapshot/u)
  assert.match(migrationPreparer, /\$radarMigration\.Snapshot/u)
