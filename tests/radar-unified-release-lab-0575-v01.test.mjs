...
test('historical reconciler proves equivalence in a disposable reference database', () => {
  ...
  assert.match(reconciler, /20260802_045057_radar_public_record_fact_value_text_v01/u)
  assert.match(reconciler, /20260802_062015_radar_public_record_evidence_role_v01/u)
  ...
  assert.match(reconciler, /factValueTextMigrationRowsRegistered: 0/u)
  assert.match(reconciler, /evidenceRoleMigrationRowsRegistered: 0/u)
...
test('executor can mutate only the disposable clone and verifies exact deltas', () => {
  ...
  assert.match(executor, /20260802_045057_radar_public_record_fact_value_text_v01/u)
  ...
  assert.match(executor, /payload_migrations 净增量不是 6/u)
  assert.match(executor, /formalMigrationsAdded = 7/u)
  assert.match(executor, /payloadMigrationsNetAdded = 6/u)
