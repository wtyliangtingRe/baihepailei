...
const scriptUrl = new URL('../scripts/radar/rehearse-unified-rating-incremental-release-9988-v01.ps1', import.meta.url)
const source = fs.readFileSync(scriptUrl, 'utf8')

test('rehearsal binds exact source and release identities', () => {
...
test('source transition and existing 575 projection are exact gates', () => {
...
test('fresh backup is restored only to a temporary PostgreSQL container', () => {
  ...
  assert.match(source, /source-before-incremental-rehearsal-\$stamp\.dump/)
...
test('storage deltas are exact and protected source state must remain unchanged', () => {
...
test('failure cleanup is explicit and automatic retry remains disabled', () => {
