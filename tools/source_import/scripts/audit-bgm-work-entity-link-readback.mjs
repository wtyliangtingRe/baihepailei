...
export const READBACK_MODE = 'payload-readback-audit/read-only/relation-fields-only';
...
function relationId(value) {
  ...
  if (typeof value === 'string') { const text = value.trim(); return text ? (/^\d+$/.test(text) ? Number(text) : text) : undefined; }
  if (typeof value === 'object') return relationId(value.id ?? value.payloadId ?? value.doc?.id ?? value.value ?? value.creator ?? value.organization);
  return undefined;
}
function normalizeIds(values) { return asArray(values).map(relationId).filter((id) => id !== undefined); }
...
function normalizeCreatorCredits(value) {
  return asArray(value).map((credit) => compactObject({
    creator: relationId(credit?.creator ?? credit?.creatorId),
    role: credit?.role ?? 'other',
    originalRole: credit?.originalRole,
    source: credit?.source,
  })).filter((credit) => credit.creator !== undefined)
    .sort((a, b) => String(a.creator).localeCompare(String(b.creator)) || String(a.role).localeCompare(String(b.role)) || String(a.originalRole ?? '').localeCompare(String(b.originalRole ?? '')));
}
function normalizeOrganizationRows(value) {
  return asArray(value).map((row) => compactObject({
    organization: relationId(row?.organization ?? row?.organizationId ?? row),
    role: row?.role ?? 'other',
    originalRole: row?.originalRole,
    source: row?.source,
  })).filter((row) => row.organization !== undefined)
    .sort((a, b) => String(a.organization).localeCompare(String(b.organization)) || String(a.role).localeCompare(String(b.role)) || String(a.originalRole ?? '').localeCompare(String(b.originalRole ?? '')));
}
function stable(value) { return JSON.stringify(value); }
function compareField(field, expected, actual) { return stable(expected) === stable(actual) ? undefined : { field, expected, actual }; }

export function expectedReadbackFieldsFromPlanItem(item) {
  const patch = buildWorkRelationPatch(item);
  return { creators: normalizeIds(patch.creators), organizations: normalizeOrganizationRows(patch.organizations), creatorCredits: normalizeCreatorCredits(patch.creatorCredits) };
}
export function actualReadbackFieldsFromPayloadWork(work) {
  return { creators: normalizeIds(work?.creators), organizations: normalizeOrganizationRows(work?.organizations), creatorCredits: normalizeCreatorCredits(work?.creatorCredits) };
}
export function compareReadbackItem(item, payloadWork) {
  ...
  const mismatches = [compareField('creators', expected.creators, actual.creators), compareField('organizations', expected.organizations, actual.organizations), compareField('creatorCredits', expected.creatorCredits, actual.creatorCredits)].filter(Boolean);
  return { status: mismatches.length === 0 ? 'pass' : 'fail', work: { payloadId: relationId(item?.work?.payloadId), title: item?.work?.title ?? payloadWork?.title ?? '', bangumiSubjectId: item?.work?.bangumiSubjectId ?? item?.work?.siteId }, mismatches };
}
export function validateReadbackPlan(plan) {
  ...
  items.forEach((item, index) => {
    if (!hasValue(item?.work?.payloadId)) errors.push({ code: 'work_payload_id_missing', message: `Missing work id for item ${index + 1}.`, index });
...
function payloadUrl(baseUrl, workId) { return `${String(baseUrl).replace(/\/$/, '')}/api/works/${encodeURIComponent(workId)}?depth=0&draft=true`; }
...
async function readPayloadWork(fetchImpl, baseUrl, token, workId) {
  const response = await fetchImpl(payloadUrl(baseUrl, workId), { method: 'GET', headers: { Authorization: `JWT ${token}` } });
...
export async function auditBgmWorkEntityLinkReadback(plan, options = {}) {
  ...
  const items = asArray(plan?.items);
  const audit = { schemaVersion: READBACK_SCHEMA_VERSION, generatedAt: new Date().toISOString(), mode: READBACK_MODE, status: 'pass', counts: { plannedWorksTotal: items.length, payloadReadsTotal: 0, matchedWorksTotal: 0, mismatchedWorksTotal: 0, errorsTotal: 0 }, errors: [], items: [] };
  if (validation.status !== 'pass') { audit.status = 'blocked'; audit.errors = validation.errors; audit.counts.errorsTotal = validation.errors.length; return audit; }
  ...
  if (audit.counts.mismatchedWorksTotal > 0 || audit.errors.length > 0) audit.status = 'fail';
  audit.counts.errorsTotal = audit.errors.length;
  return audit;
}
export function renderReadbackAuditMarkdown(audit) {
  const lines = ['# BGM Work Entity Link Readback Audit', '', `- generatedAt: ${audit.generatedAt}`, `- mode: ${audit.mode}`, `- status: ${audit.status}`, '', '## Counts', '', '| key | value |', '| --- | ---: |'];
  ...
  if (failed.length > 0) { lines.push('', '## Mismatches'); for (const item of failed) { lines.push(`- ${item.index + 1}: ${item.work.title || item.work.payloadId}`); for (const mismatch of item.mismatches) lines.push(`  - ${mismatch.field}: expected ${stable(mismatch.expected)}, actual ${stable(mismatch.actual)}`); } }
  if (audit.errors.length > 0) { lines.push('', '## Errors'); for (const error of audit.errors) lines.push(`- ${error.code}: ${error.message}`); }
  lines.push('');
  return `${lines.join('\n')}\n`;
}
async function main() {
