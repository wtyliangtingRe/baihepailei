...
function main() {
  ...
  if (args.execute || args.apply || args.write || args.patch || args.confirm || args.gate || args['approval-token']) {
    throw new Error('Assessment handoff assembly is local-only. Execute/apply/write/gate flags are rejected.')
  }
  ...
  if (!fs.existsSync(handoffManifestFile)) throw new Error(`Handoff manifest not found: ${handoffManifestFile}`)

  const handoff = readJson(handoffManifestFile)
  const blockers = []
  const warnings = []
  if (val(handoff?.version) !== ASSESSMENT_HANDOFF_VERSION) blockers.push('handoff_version_mismatch')
  if (val(handoff?.batchId) !== batchId) blockers.push('handoff_batch_id_mismatch')
  if (val(handoff?.queue) !== 'ready_for_ai_assessment') blockers.push('handoff_queue_not_assessment')
  if (!fs.existsSync(val(handoff?.copiedSourceFile))) blockers.push('handoff_source_copy_missing')
  else if (sha256File(val(handoff.copiedSourceFile)) !== val(handoff?.copiedSourceSha256)) blockers.push('handoff_source_copy_sha256_mismatch')
  ...
  for (const chunk of Array.isArray(handoff?.chunks) ? handoff.chunks : []) {
    ...
    if (!chunkBlockers.length) {
      ...
      if (!validated.blockers.length) allMergedRows.push(...validated.mergedRows)
    }
  ...
  if (!Array.isArray(handoff?.chunks) || handoff.chunks.length !== Number(handoff?.chunkCount)) blockers.push('handoff_chunk_count_mismatch')
  if (!blockers.length && allMergedRows.length !== Number(handoff?.rowCount)) blockers.push('assembled_row_count_mismatch')
  ...
  const summary = {
    ...
    nextStep: complete
      ? `pnpm radar:resolve -- --input "${completeFile}" --out-dir "${resolverOutDir}"`
      : 'Complete or correct the response files listed in blockers, then rerun assembly.',
  }
