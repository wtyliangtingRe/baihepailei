...
$WorkingRoot = [System.IO.Path]::GetFullPath((Join-Path $DataLocal "working\ai-radar\incremental-wave-0004-2500-v01"))
...
function Get-ValidFullCatalog([System.IO.FileInfo]$File) {
  try {
    ...
    return [pscustomobject]@{ File = $File; Rows = $Rows; GeneratedAt = $GeneratedAt; LastWriteTimeUtc = $File.LastWriteTimeUtc }
  }
  catch { return $null }
}
...
Write-Host "[2/7] 核验第三波生成的增量处理台账" -ForegroundColor Cyan
$LedgerFile = if ([string]::IsNullOrWhiteSpace($Ledger)) { $DefaultLedger } else { Assert-UnderDataLocal (Resolve-RepoPath $Ledger) }
if (-not (Test-Path -LiteralPath $LedgerFile)) { throw "增量台账不存在，请先完成第三波检查点：$LedgerFile" }
$LedgerRows = @(
...
Write-Host "[3/7] 增量选择 2500 条联网研究容量并分离重新评级队列" -ForegroundColor Cyan
...
if ($SelectionSummary.safety.payloadWrite -ne $false -or $SelectionSummary.safety.directPostgresqlWrite -ne $false -or [int]$SelectionSummary.safety.humanTrackMutations -ne 0) {
  throw "增量选择安全声明不符合要求"
}

Write-Host "[4/7] 运行增量台账、2500 拆分与恢复规则测试" -ForegroundColor Cyan
& node --test `
...
if (@($PackageManifest.subwaves | Where-Object { [int]$_.rowCount -ne $SubwaveSize -or [int]$_.chunkCount -ne $ExpectedChunksPerFullSubwave }).Count -ne 0) {
  throw "存在不是 250 条或不是 50 块的子波次"
}
if ($PackageManifest.safety.payloadWrite -ne $false -or $PackageManifest.safety.directPostgresqlWrite -ne $false -or [int]$PackageManifest.safety.humanTrackMutations -ne 0) {
...
Write-Host "[7/7] 2500 条增量输入包完成" -ForegroundColor Cyan
[pscustomobject]@{
  WaveId = $WaveId
