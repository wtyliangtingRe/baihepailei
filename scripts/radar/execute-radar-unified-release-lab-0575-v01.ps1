...
try {
  ...
  if (
    ...
    [int]$reconciliation.evidenceRoleMigrationRowsRegistered -ne 0 -or
  ...
  Write-Host ''
  Write-Host '==> 只在临时 PostgreSQL 执行 Records、Ratings、事实文本与证据角色正式迁移' -ForegroundColor Green
  ...
  foreach ($key in $beforeCounts.Keys) {
    ...
    if ($key -eq 'public.audit_events') {
      ...
      if ($after -ne $before + 6) { throw "payload_migrations 净增量不是 6：$before -> $after" }
  ...
  Write-JsonFile (Join-Path $outDir 'lab-acceptance.json') ([ordered]@{
    ...
    formalMigrationsAdded = 7
    developmentMigrationMarkersRemoved = 1
    payloadMigrationsNetAdded = 6
