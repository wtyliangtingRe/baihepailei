...
if ($LASTEXITCODE -ne 0) { throw '隔离演练 SQL v01 builder 语法检查失败。' }
node --check '.\scripts\radar\build-radar-public-conclusions-lab-sql-v02.mjs'
if ($LASTEXITCODE -ne 0) { throw '隔离演练 SQL v02 builder 语法检查失败。' }
node --test `
...
if ($LASTEXITCODE -ne 0 -or -not [string]::IsNullOrWhiteSpace($productionRadarTable)) {
  throw '生产库已存在 radar_public；当前首次建表演练输入失效。'
}
...
$evidencePackDir = Join-Path $tempRoot 'evidence-pack'
New-Item -ItemType Directory -Path $schemaReviewDir, $auditDir, $sqlDir, $evidencePackDir -Force | Out-Null
...
node '.\scripts\radar\build-radar-public-conclusions-lab-sql-v02.mjs' `
  --audit-dir $auditDir `
...
try {
  ...
  if ($LASTEXITCODE -ne 0) { throw '复制表行数 SQL 到生产容器失败。' }
  ...
  if ($LASTEXITCODE -ne 0) { throw '复制 fresh backup 到隔离容器失败。' }
  docker cp $countSqlPath "${labContainer}:$containerCountsPath" | Out-Null
  if ($LASTEXITCODE -ne 0) { throw '复制计数 SQL 到隔离容器失败。' }
  ...
  if ($LASTEXITCODE -ne 0) { throw '复制完整 ready JSONL 到隔离容器失败。' }
  foreach ($copy in @(
    @((Join-Path $sqlDir 'radar-public-lab-apply.sql.lab-only'), $containerApplyPath),
    @((Join-Path $sqlDir 'radar-public-lab-acceptance.sql'), $containerAcceptancePath),
    @((Join-Path $sqlDir 'radar-public-lab-rollback.sql.lab-only'), $containerRollbackPath),
    @((Join-Path $sqlDir 'radar-public-lab-post-rollback-acceptance.sql'), $containerPostRollbackPath)
  )) {
  ...
  Write-JsonFile $stageStatusPath $stageStatus
  $evidenceFiles = @(Get-ChildItem -LiteralPath $outDir -Recurse -File | Where-Object { $_.FullName -ne $backupPath -and $_.Name -ne 'manifest.json' } | Sort-Object FullName)
  ...
  Get-ChildItem -LiteralPath $outDir -Force | Where-Object { $_.Name -ne 'database-backup.dump' } | ForEach-Object {
    Copy-Item -LiteralPath $_.FullName -Destination $evidencePackDir -Recurse -Force
  }
  Compress-Archive -Path (Join-Path $evidencePackDir '*') -DestinationPath $bundlePath -CompressionLevel Optimal -Force
  ...
  Write-Host 'LabContainerRemoved      : True'
  ...
  if (-not $packageCompleted -and (Test-Path -LiteralPath $stageStatusPath -PathType Leaf)) {
    Write-JsonFile $stageStatusPath $stageStatus
  }
