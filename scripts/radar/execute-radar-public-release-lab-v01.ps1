...
function Get-DotEnvValue([string[]]$Names) {
  foreach ($name in $Names) {
    ...
    $line = Get-Content -LiteralPath '.env' -Encoding UTF8 | Where-Object { $_ -match "^\s*$escaped\s*=" } | Select-Object -First 1
...
function Invoke-LabSql([object]$Lab, [string]$HostFile, [string]$ContainerFile, [string]$OutputFile, [string]$ErrorFile) {
  ...
  & docker exec -e "PGPASSWORD=$($Lab.labPassword)" $Lab.labContainer `
    psql -X -qAt -v ON_ERROR_STOP=1 -U $Lab.labUser -d $Lab.labDatabase -f $ContainerFile `
  ...
  do {
    ...
    try {
      ...
      if (
        ...
        [string]$marker.researchCommit -eq [string]$Lab.researchCommit
      ) { return $marker }
...
$containerRunning = ([string](& docker inspect -f '{{.State.Running}}' $lab.labContainer)).Trim()
...
if ($head -ne $ExpectedWebsiteHead -or (git rev-parse "origin/$ExpectedBranch").Trim() -ne $ExpectedWebsiteHead) {
  throw "网站实验提交不符合预期：$head"
}
...
try {
  ...
  if ($LASTEXITCODE -ne 0 -or (git rev-parse HEAD).Trim() -ne [string]$lab.researchCommit) {
    throw '研究仓库不再位于实验环境锁定提交。'
  }
...
try {
  ...
  & docker exec -e "PGPASSWORD=$($lab.labPassword)" $lab.labContainer psql -X -qAt -v ON_ERROR_STOP=1 `
    -U $lab.labUser -d $lab.labDatabase `
  ...
  Invoke-WithEnvironment @{
    ...
    RADAR_PUBLIC_RELEASE_LAB_RESEARCH_COMMIT = [string]$lab.researchCommit
  } {
  ...
  Invoke-WithEnvironment @{
    ...
    node '.\scripts\radar\run-public-release-lab-import-v01.mjs' `
      ...
      --expected-research-commit ([string]$lab.researchCommit) `
  ...
  $countSql = Join-Path (Split-Path -Parent $environmentPath) 'table-counts.sql'
  $fingerprintSql = Join-Path (Split-Path -Parent $environmentPath) 'protected-fingerprints.sql'
  ...
  if (($newKeys -join "`n") -ne (@($expectedNew.Keys | Sort-Object) -join "`n")) {
  ...
  Write-JsonFile (Join-Path $outDir 'lab-acceptance.json') ([ordered]@{
    ...
    researchCommit = [string]$lab.researchCommit
    publicRecords = 520
  ...
  $appProcess = $null
  & docker rm -f $lab.labContainer | Out-Null
  if ($LASTEXITCODE -ne 0) { throw '销毁实验 PostgreSQL 容器失败。' }
  $containerRemoved = $true
  Remove-Item -LiteralPath ([string]$lab.backupPath) -Force -ErrorAction SilentlyContinue
  Remove-Item -LiteralPath $environmentPath -Force -ErrorAction SilentlyContinue

  Copy-Item -LiteralPath (Join-Path $outDir '*') -Destination $evidenceDir -Recurse -Force
  Copy-Item -LiteralPath ([string]$lab.sourceTableCounts), ([string]$lab.sourceProtectedFingerprints), ([string]$lab.restoredTableCounts), ([string]$lab.restoredProtectedFingerprints) -Destination $evidenceDir -Force
  Write-Manifest $evidenceDir
  Compress-Archive -LiteralPath (Join-Path $evidenceDir '*') -DestinationPath $bundlePath -CompressionLevel Optimal -Force
  ...
  Stop-LabApp $appProcess
  if (-not $containerRemoved) { & docker rm -f $lab.labContainer 2>$null | Out-Null }
  Remove-Item -LiteralPath ([string]$lab.backupPath) -Force -ErrorAction SilentlyContinue
  Remove-Item -LiteralPath $environmentPath -Force -ErrorAction SilentlyContinue
  if (-not $success) {
    Write-Host ''
