param(
  ...
  [ValidateSet('RUN-ISOLATED-RADAR-BLOCKED-INCREMENTAL-STORAGE-LAB-V01')]
  [string]$Confirm,
  [string]$ExpectedAssemblySHA256 = 'D41FB41951E35E8DC0C2CDDE1FB904968AF3DF5D3564BDDB8A13A3FB002C14C2',
...
$ExpectedFinalRows = 9683
$ExpectedReadySHA256 = '4DDAADC268B9F84F7B8A0D2C9BA8A2E73C37F129967CCF0D59C7E970B3066862'
$AllowedDirtyFiles = @(
  'next-env.d.ts',
  'payload-types.ts',
  'docs/guides/radar-blocked-incremental-storage-lab-v01.md',
  'scripts/radar/build-radar-blocked-incremental-storage-lab-v01.mjs',
  'scripts/radar/run-and-package-radar-blocked-incremental-storage-lab-v01.ps1',
  'tests/radar-blocked-incremental-storage-lab.test.mjs'
)
...
if ($Confirm -ne 'RUN-ISOLATED-RADAR-BLOCKED-INCREMENTAL-STORAGE-LAB-V01') {
  throw '隔离演练确认字符串不匹配。'
}
...
node --check '.\scripts\radar\build-radar-blocked-incremental-storage-lab-v01.mjs'
if ($LASTEXITCODE -ne 0) { throw '增量 lab SQL builder 语法检查失败。' }
node --test `
  '.\tests\radar-blocked-incremental-storage-lab.test.mjs' `
  '.\tests\radar-blocked-assembly-dryrun.test.mjs' `
...
$outDir = Join-Path $repoRoot "exports\radar-blocked-incremental-storage-lab-$stamp"
$evidenceDir = Join-Path $repoRoot "exports\radar-blocked-incremental-storage-lab-evidence-$stamp"
$evidenceBundle = Join-Path $repoRoot "exports\RADAR-BLOCKED-INCREMENTAL-STORAGE-LAB-EVIDENCE-$stamp.zip"
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) (
  "radar-blocked-incremental-storage-lab-$stamp-" +
...
if ((Get-FileHash -LiteralPath $readyPath -Algorithm SHA256).Hash -ne $ExpectedReadySHA256) {
  throw 'Assembly storage-ready JSONL SHA-256 不匹配。'
}

node '.\scripts\radar\build-radar-blocked-incremental-storage-lab-v01.mjs' `
  --assembly-dir $assemblyDir `
  --assembly-zip-sha256 $assemblyHash `
  --out-dir $sqlDir
if ($LASTEXITCODE -ne 0) { throw '增量 storage lab SQL 构建失败。' }

$planSummaryPath = Join-Path $sqlDir 'radar-blocked-incremental-storage-lab-plan-summary.json'
...
if (
  ...
  throw '增量 lab plan summary 不符合固定门槛。'
}
...
$sourcePrefix = "/tmp/radar-blocked-incremental-storage-lab-$stamp"
...
$labContainer = "baihepailei-radar-blocked-incremental-lab-$stamp"
$labUser = 'radar_lab'
$labDatabase = 'radar_lab'
$labPassword = [Guid]::NewGuid().ToString('N')
$containerDumpPath = '/tmp/database-backup.dump'
$containerReadyPath = '/tmp/radar-blocked-incremental-storage-ready.jsonl'
...
try {
  ...
  Write-Host ''
  Write-Host '==> 在隔离数据库录入 683 条 storage-normalized 公共结论' -ForegroundColor Cyan
  ...
  $labSummary = [ordered]@{
    schemaVersion = 1
    generatedAt = [DateTime]::UtcNow.ToString('o')
    version = 'radar-blocked-incremental-storage-lab-runner-v0.1'
    branch = $ExpectedBranch
    head = $localHead
    assemblyBundleSha256 = $assemblyHash.ToLowerInvariant()
    assemblyReadySha256 = $ExpectedReadySHA256.ToLowerInvariant()
    ...
    nextStep = 'Upload the evidence ZIP for independent validation before a production gate is created.'
  }
  Write-JsonFile `
    -Path (Join-Path $outDir 'radar-blocked-incremental-storage-lab-summary.json') `
    -Value $labSummary
  ...
  Write-Host ''
  Write-Host '683 条增量 storage 隔离录入与 exact rollback 已完成～' -ForegroundColor Green
