...
const VERSION = 'ai-radar-v06-version-roundtrip-lab-v0.2'
const CONFIRMATION = 'EXECUTE-V06-VERSION-ROUNDTRIP-LAB-V02-ONLY'
const DEFAULT_OUT_ROOT = 'data_local/staging/ai-radar/v06-version-roundtrip-lab-v02'
...
export function normalizedDocumentStateIgnoringRootStatus(document) {
...
export function matchingPublishedBaselineVersions(versionRows, publishedDocument) {
  const expected = normalizedDocumentStateIgnoringRootStatus(publishedDocument)
  return list(versionRows)
    .filter((row) => equal(
      normalizedDocumentStateIgnoringRootStatus(row?.version),
      expected,
    ))
    .sort((a, b) => Date.parse(val(b?.createdAt)) - Date.parse(val(a?.createdAt)))
}

export function matchingExactVersions(versionRows, document) {
...
export async function runLabV02(options) {
  const {
    baseUrl,
    candidate,
  ...
  if (initialBlockers.length) throw new Error(`Version roundtrip lab v0.2 blocked: ${initialBlockers.join(', ')}`)
  ...
  const outputs = {
    ...
    afterCleanRestore: path.join(outDir, 'after-clean-baseline-restore.json'),
  ...
  writeEvidence(outputs.discovery, {
    ...
    matchingDraftVersions: draftMatches.map((row) => ({
      id: val(row?.id),
      createdAt: val(row?.createdAt),
      status: val(row?.version?._status),
    })),
  ...
  const publishedAfterCleanRestore = await readWork(baseUrl, token, targetId)
  const draftAfterCleanRestore = await readWork(baseUrl, token, targetId, { draft: true })
  const afterCleanRestore = {
    published: stateSnapshot(publishedAfterCleanRestore, patch),
    draft: stateSnapshot(draftAfterCleanRestore, patch),
  }
  const cleanRestoreBlockers = unique([
    ...(afterCleanRestore.published.stateSha256 !== baseline.published.stateSha256
      ? ['lab_clean_restore_changed_published_main'] : []),
    ...(afterCleanRestore.draft.stateIgnoringRootStatusSha256 !== baseline.published.stateIgnoringRootStatusSha256
      ? ['lab_clean_restore_latest_draft_not_equal_published_content'] : []),
    ...(afterCleanRestore.draft.status !== 'draft'
      ? ['lab_clean_restore_latest_status_not_draft'] : []),
    ...(afterCleanRestore.draft.unrelatedStateSha256 !== baseline.published.unrelatedStateSha256
      ? ['lab_clean_restore_changed_unrelated_state'] : []),
    ...(afterCleanRestore.draft.humanStateSha256 !== baseline.published.humanStateSha256
      ? ['lab_clean_restore_changed_human_state'] : []),
  ])
  writeEvidence(outputs.afterCleanRestore, {
    generatedAt: new Date().toISOString(),
    afterCleanRestore,
    blockers: cleanRestoreBlockers,
  })
  if (cleanRestoreBlockers.length) throw new Error(`Lab clean restore failed: ${cleanRestoreBlockers.join(', ')}`)
  ...
  const afterRadar = {
    ...
    patchMatchedDraft: patchMatches(draftAfterRadar, patch),
  }
  const afterRadarBlockers = unique([
    ...(!afterRadar.patchMatchedPublished ? ['lab_radar_patch_not_present_in_published'] : []),
    ...(!afterRadar.patchMatchedDraft ? ['lab_radar_patch_not_present_in_latest_version'] : []),
  ...
  const finalBlockers = unique([
    ...
    ...(final.published.stateSha256 !== afterRadar.published.stateSha256
      ? ['lab_restoring_original_draft_changed_published_main'] : []),
  ...
  const summary = {
    ...
    safety: {
      ...
      publishedBaselineMatchingIgnoresOnlyRootStatus: true,
      originalDraftMatchingRemainsExact: true,
    },
  }
  writeEvidence(outputs.summary, summary)
  return summary
