...
const VERSION = 'ai-radar-direct-overwrite-failure-forensics-v0.2'
const DEFAULT_OUT_ROOT = 'data_local/staging/ai-radar/full-coverage-direct-overwrite-v02'
const DEFAULT_REPORT_ROOT = 'data_local/outputs/ai-radar/forensics/direct-overwrite-failure-v02'
...
function findFailedRun(outRoot, targetId) {
  for (const ledgerFile of findFiles(outRoot, 'execution-ledger.jsonl')) {
    const events = readJsonl(ledgerFile)
    const failureEvent = [...events]
      .reverse()
      .find((event) => val(event?.targetId) === targetId && val(event?.status) === 'execution_failed')
    if (!failureEvent) continue
    const runDir = path.dirname(ledgerFile)
    const planFile = path.join(runDir, 'publication-plan.jsonl')
    const planRows = readJsonl(planFile)
    const planRow = planRows.find((row) => val(row?.targetId) === targetId) || null
    return {
      runDir,
      ledgerFile,
      planFile: fs.existsSync(planFile) ? planFile : null,
...
function normalizeForPathDiff(value, { root = false } = {}) {
  if (Array.isArray(value)) {
    return value.map((item) => normalizeForPathDiff(item))
  }
  if (value && typeof value === 'object') {
    const entries = []
...
function selectBaselineVersion(versions, planRow, patch) {
  ...
  const exact = infos.find(({ info }) => (
    val(expected?.publishedStateSha256) && info.stateSha256 === val(expected.publishedStateSha256)
  ))
  ...
  if (unrelatedAndHuman) return { ...unrelatedAndHuman, matchKind: 'unrelated_and_human_sha256' }
...
function sourceFileInfo(file) {
  return file && fs.existsSync(file)
    ? { path: file, sha256: fileSha256(file), bytes: fs.statSync(file).size }
    : null
}
...
async function runForensics(options) {
  ...
  const failedRun = findFailedRun(outRoot, targetId)
  if (!failedRun) throw new Error(`No failed direct-overwrite ledger event found for Work ${targetId}`)
  if (!failedRun.planRow?.patch) throw new Error(`Failed run has no publication patch for Work ${targetId}`)

  const token = await login(baseUrl, email, password)
  const [published, draft, versions] = await Promise.all([
    readWork(baseUrl, token, targetId),
    readWork(baseUrl, token, targetId, { draft: true }),
    readVersions(baseUrl, token, targetId),
  ])

  const patch = failedRun.planRow.patch
  const baseline = selectBaselineVersion(versions, failedRun.planRow, patch)
  const allDiffs = baseline ? diffDocuments(baseline.row.version, published) : []
  const categories = categorizeDiffs(allDiffs, patch)
  const currentPublishedStateSha256 = sha256(normalizedDocumentState(published))
  const currentDraftStateSha256 = sha256(normalizedDocumentState(draft))
  const currentPublishedUnrelatedStateSha256 = sha256(unrelatedPublishedState(published, patch))
  const currentPublishedHumanStateSha256 = sha256(selectedHumanState(published))

  const backupProofFile = newestMatching(backupRoot, /^RADAR-DIRECT-OVERWRITE-FIRST5-.*-proof\.json$/u)
  ...
  const report = canonical({
    ...
    failedRun: {
      ...
      ledger: sourceFileInfo(failedRun.ledgerFile),
      plan: sourceFileInfo(failedRun.planFile),
      planSummary: sourceFileInfo(failedRun.planSummaryFile),
      failureEvent: failedRun.failureEvent,
      planRow: failedRun.planRow,
    },
    backup: backupProof ? {
    ...
    current: {
      ...
      publishedStateSha256: currentPublishedStateSha256,
      draftStateSha256: currentDraftStateSha256,
      publishedAndDraftStateEqual: currentPublishedStateSha256 === currentDraftStateSha256,
      publishedUnrelatedStateSha256: currentPublishedUnrelatedStateSha256,
      publishedHumanStateSha256: currentPublishedHumanStateSha256,
      expectedBefore: failedRun.planRow.expectedBefore || null,
      publishedUnrelatedMatchesExpectedBefore: (
        currentPublishedUnrelatedStateSha256
        === val(failedRun.planRow?.expectedBefore?.publishedUnrelatedStateSha256)
      ),
      publishedHumanMatchesExpectedBefore: (
        currentPublishedHumanStateSha256
        === val(failedRun.planRow?.expectedBefore?.publishedHumanStateSha256)
      ),
    },
    diff: {
  ...
  const reportFile = path.join(reportRoot, `${stamp}-work-${targetId}-direct-overwrite-diff.json`)
...
async function main() {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    targetId,
    payloadDataMutation: false,
    patchPresentInPublished: report.current.patchPresentInPublished,
    baselineFound: report.baseline.found,
    baselineMatchKind: report.baseline.matchKind,
    changedPathCounts: report.diff.counts,
    unrelatedChangedPaths: report.diff.unrelatedChangedPaths.map((item) => item.path),
  }, null, 2)}\n`)
}
