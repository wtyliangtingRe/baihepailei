...
const VERSION = 'ai-radar-wave3-ai-qa-final-v0.1'
const BATCH_ID = 'RADAR-ASSESS-RESEARCH-0250'
...
function main() {
  ...
  for (const file of [safeDecisionsFile, safeTargetedResearchFile, safeResolvedFile, safeAssemblySummaryFile, safeResolverSummaryFile]) {
    if (!fs.existsSync(file)) throw new Error(`Required Wave 3 finalization input not found: ${file}`)
  }
  ...
  if (assemblySummary?.complete !== true || Number(assemblySummary?.assembledRows) !== expectedRows) {
    throw new Error('Wave 3 calibrated assembly is incomplete or has the wrong row count')
  }
  if (Number(resolverSummary?.rowsRead) !== expectedRows) throw new Error('Wave 3 resolver row count mismatch')
  if (Number(resolverSummary?.safety?.humanTrackMutations ?? 0) !== 0) throw new Error('Wave 3 resolver reported a human-track mutation')
  ...
  for (const row of resolvedRows) {
    ...
    if (!workId || resolvedById.has(workId)) throw new Error(`Invalid or duplicate resolved workId: ${workId}`)
  ...
  for (const decision of decisions) {
    ...
    if (!workId || !siteId || seenDecisionIds.has(workId)) throw new Error(`Invalid or duplicate AI QA decision workId: ${workId}`)
    ...
    if (!resolved || val(resolved?.siteId) !== siteId) throw new Error(`Resolved identity mismatch for ${workId}`)
  ...
  if (passed.length !== expectedPassed || deferred.length !== expectedDeferred) {
    throw new Error(`Unexpected Wave 3 AI QA counts: passed=${passed.length}, deferred=${deferred.length}`)
  }
  ...
  const outputs = {
    all: path.join(safeOutputDir, 'ai-radar-wave3-ai-qa-final-v0.1.jsonl'),
    passed: path.join(safeOutputDir, 'ai-radar-wave3-ai-qa-passed-v0.1.jsonl'),
    deferred: path.join(safeOutputDir, 'ai-radar-wave3-ai-qa-deferred-v0.1.jsonl'),
    decisionsCsv: path.join(safeOutputDir, 'ai-radar-wave3-ai-qa-decisions-v0.1.csv'),
    targetedResearch: path.join(safeOutputDir, 'ai-radar-wave3-targeted-research-v0.1.jsonl'),
    summary: path.join(safeOutputDir, 'ai-radar-wave3-ai-qa-final-summary-v0.1.json'),
    report: path.join(safeOutputDir, 'AI_RADAR_WAVE3_AI_QA_FINAL_REPORT_v0.1.md'),
  }
  ...
  const headers = ['workId','siteId','title','aiQaStatus','currentGradeSuggestion','decisiveRuleCode','confidencePercent','evidenceCoveragePercent','evidenceStatus','aiQaReasons','humanReviewStatus','humanTrackAction']
  const lines = [headers.map(csvCell).join(',')]
  for (const row of finalRows) {
    lines.push([row.workId,row.siteId,row.title,row.aiQaStatus,row.currentGradeSuggestion,row.decisiveRule?.code,row.confidencePercent,row.evidenceCoveragePercent,row.evidenceStatus,row.aiQaReasons,row.humanReviewStatus,row.humanTrackAction].map(csvCell).join(','))
  }
  fs.writeFileSync(outputs.decisionsCsv, `${lines.join('\n')}\n`, 'utf8')
  ...
  const summary = {
    ...
    nextStep: 'Build the incremental processing ledger and upload the Wave 3 checkpoint ZIP.',
  }
  writeJson(outputs.summary, summary)
  const report = [
    '# AI Radar 第三波 AI QA 最终报告 v0.1',
    '',
    `- 批次：${summary.batchId}`,
    ...
    '## 暂缓条目',
    '',
    ...(deferred.length ? deferred.map((row) => `- ${row.workId}｜${row.title}｜${row.currentGradeSuggestion}/${val(row.decisiveRule?.code)}`) : ['- none']),
    '',
  ].join('\n')
