...
$NodeScript = Join-Path $PSScriptRoot "inspect-ai-radar-direct-overwrite-failure-v02.mjs"
$TestFile = Join-Path $RepoRoot "tests\radar-direct-overwrite-failure-forensics-v02.test.mjs"
$ReportRoot = Join-Path $RepoRoot "data_local\outputs\ai-radar\forensics\direct-overwrite-failure-v02"
$CheckpointRoot = Join-Path $RepoRoot "data_local\outputs\ai-radar\checkpoints"
$ZipPath = Join-Path $CheckpointRoot "RADAR-FULL-COVERAGE-DIRECT-OVERWRITE-FAILURE-FORENSICS-v02.zip"
...
if ($CurrentBranch -ne $ExpectedBranch) {
  throw "direct-overwrite 失败取证必须在 $ExpectedBranch 分支执行；当前为 $CurrentBranch"
}

foreach ($Required in @($NodeScript, $TestFile)) {
  if (-not (Test-Path -LiteralPath $Required)) {
    throw "缺少取证文件：$Required"
  }
}
...
$TestOutput = Join-Path $env:TEMP ("radar-direct-overwrite-failure-forensics-tests-" + [guid]::NewGuid().ToString("N") + ".txt")
$Temp = $null
try {
  ...
  if ($TestExit -ne 0) {
    throw "direct-overwrite 失败取证回归测试失败"
  }

  Ensure-Credentials

  $BeforeReports = @(
    Get-ChildItem -LiteralPath $ReportRoot -File -Filter "*-work-$TargetId-direct-overwrite-diff.json" -ErrorAction SilentlyContinue |
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "direct-overwrite 失败只读取证脚本失败"
  }

  $ReportFile = Get-ChildItem -LiteralPath $ReportRoot -File -Filter "*-work-$TargetId-direct-overwrite-diff.json" |
    Where-Object { $_.FullName -notin $BeforeReports } |
    Sort-Object LastWriteTimeUtc -Descending |
    Select-Object -First 1

  if (-not $ReportFile) {
    throw "没有找到本次 direct-overwrite 失败取证报告"
  }
  ...
  $Temp = Join-Path ([System.IO.Path]::GetTempPath()) ("radar-direct-overwrite-failure-forensics-" + [guid]::NewGuid().ToString("N"))
  New-Item -ItemType Directory -Path $Temp | Out-Null

  Copy-Item -LiteralPath $ReportFile.FullName -Destination (Join-Path $Temp "direct-overwrite-failure-report.json")
  Copy-Item -LiteralPath $TestOutput -Destination (Join-Path $Temp "test-output.txt")

  $Manifest = [ordered]@{
    version = "ai-radar-direct-overwrite-failure-forensics-checkpoint-v0.2"
    generatedAt = (Get-Date).ToUniversalTime().ToString("o")
    gitCommit = (git rev-parse HEAD).Trim()
    targetId = $TargetId
    reportFile = "direct-overwrite-failure-report.json"
