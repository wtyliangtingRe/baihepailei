...
$outDir = Join-Path $repoRoot "exports\test-work-merge-dryrun-$stamp"
$bundlePath = Join-Path $repoRoot "exports\TEST-WORK-MERGE-DRYRUN-$stamp.zip"
...
if ($LASTEXITCODE -ne 0) {
  throw 'Test Work merge dry-run 生成失败。'
}
...
if ($missing.Count -gt 0) {
  throw "Test Work merge dry-run 输出不完整：$($missing -join ', ')"
}
...
if ($summary.safety.databaseWrite -ne $false -or
  ...
  throw 'Test Work merge dry-run 未证明安全边界。'
}
...
Write-Host ''
Write-Host 'Test Work merge dry-run 与打包完成' -ForegroundColor Green
