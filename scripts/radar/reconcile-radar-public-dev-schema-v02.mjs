...
FROM (VALUES ('${RECORDS_BASELINE}'), ('${RECORDS_MIGRATION}'), ('${RATINGS_MIGRATION}')) AS wanted(name)
...
DO $$
DECLARE dev_count integer; historical_count integer; records_count integer; ratings_count integer;
BEGIN
  ...
  IF dev_count <> 1 OR historical_count <> 0 OR records_count <> 0 OR ratings_count <> 0 THEN
    RAISE EXCEPTION 'reconciliation precondition failed: dev %, historical %, records %, ratings %', dev_count, historical_count, records_count, ratings_count;
  END IF;
END $$;
DELETE FROM public.payload_migrations WHERE name = 'dev' AND batch = -1;
