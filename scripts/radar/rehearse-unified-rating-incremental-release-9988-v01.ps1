param(
  ...
  [Parameter(Mandatory = $true)][ValidateSet('REHEARSE-RADAR-UNIFIED-RATING-INCREMENTAL-RELEASE-9988-V01')][string]$Confirm,
  ...
  [string]$SourceUrl = 'http://127.0.0.1:3000',
  [string]$PayloadEmail = '',
...
if ($Confirm -ne 'REHEARSE-RADAR-UNIFIED-RATING-INCREMENTAL-RELEASE-9988-V01') {
  throw '确认字符串不匹配。'
}
...
$script:outDir = Join-Path $repoRoot "data_local\outputs\radar-unified-rating-incremental-release-9988-v01\rehearsal-$stamp"
$backupDir = Join-Path $repoRoot 'data_local\backups\radar-unified-rating-incremental-release-9988-v01'
$evidenceDir = Join-Path $repoRoot "exports\radar-unified-rating-incremental-release-9988-$stamp"
$evidenceZip = Join-Path $repoRoot "exports\RADAR-UNIFIED-RATING-INCREMENTAL-REHEARSAL-9988-$stamp.zip"
...
$stage = [ordered]@{
  schemaVersion = 'radar-unified-rating-incremental-rehearsal-stage-9988-v01'
  startedAt = [DateTime]::UtcNow.ToString('o')
  toolHead = $ExpectedToolHead
  researchHead = $ExpectedResearchHead
  releaseId = $ReleaseId
  sourcePlanPassed = $false
...
$tempContainer = "baihepailei-radar-incremental-rehearsal-$stamp"
...
$operationError = $null
$freshBackupPath = Join-Path $backupDir "source-before-incremental-rehearsal-$stamp.dump"
$freshBackupContainerPath = "/tmp/source-before-incremental-rehearsal-$stamp.dump"
$candidatePath = Join-Path $outDir 'radar-unified-rating-incremental-rehearsal-candidate-9988-v01.json'
$candidateSha256 = $null

try {
  $sourceHealth = Invoke-WebRequest -UseBasicParsing -Uri "$($SourceUrl.TrimEnd('/'))/api/works?limit=1&depth=0" -Method Get -TimeoutSec 20
  if ($sourceHealth.StatusCode -ne 200) { throw "源 Payload 服务不可用：HTTP $($sourceHealth.StatusCode)" }

  $sourcePlanDir = Join-Path $outDir 'source-readonly-transition-plan'
  $nodePlanCode = @'
...
'@
  Invoke-RadarWithEnvironment -Variables @{
    ...
    node --input-type=module -e $nodePlanCode -- `
      --input $releaseDirectory `
      --url $SourceUrl `
      --out-dir $sourcePlanDir `
      --expected-main-head $ExpectedBaseMain `
      --confirm 'PLAN-RADAR-UNIFIED-RATING-INCREMENTAL-RELEASE-9988-V01'
    if ($LASTEXITCODE -ne 0) { throw '源库只读 transition plan 失败。' }
  }
  $sourcePlanSummaryPath = Join-Path $sourcePlanDir 'transition-summary.json'
  $sourcePlanSummary = Get-Content -LiteralPath $sourcePlanSummaryPath -Raw -Encoding UTF8 | ConvertFrom-Json -Depth 100
  if (
    $sourcePlanSummary.accepted -ne $true -or
    [int]$sourcePlanSummary.sourceState.works -ne $ExpectedWorks -or
    [int]$sourcePlanSummary.sourceState.currentPublicRecords -ne $ExpectedExistingRecords -or
    [int]$sourcePlanSummary.sourceState.currentPublicRatings -ne $ExpectedExistingRatings -or
    [int]$sourcePlanSummary.transition.rows -ne 9988 -or
    [int]$sourcePlanSummary.transition.blockers -ne 0 -or
    [int]$sourcePlanSummary.transition.recordStatusCounts.ready_create -ne 9988 -or
    [int]$sourcePlanSummary.transition.ratingStatusCounts.ready_create -ne 9988 -or
    [int]$sourcePlanSummary.transition.updates -ne 0 -or
    [int]$sourcePlanSummary.transition.deletes -ne 0
  ) { throw '源库 transition plan 未满足精确 9,988 + 9,988 create-only 门。' }
  $stage.sourcePlanPassed = $true
  Write-RadarJsonFile $stagePath $stage
  ...
  $criticalCodePaths = @(
    ...
    'scripts/radar/rehearse-unified-rating-incremental-release-9988-v01.ps1',
  ...
  $candidate = [ordered]@{
    schemaVersion = 'radar-unified-rating-incremental-rehearsal-candidate-9988-v01'
    accepted = $true
    ...
    source = [ordered]@{
      ...
      transitionSummaryPath = $sourcePlanSummaryPath
      transitionSummarySha256 = Get-FileSha $sourcePlanSummaryPath
    }
    freshBackup = [ordered]@{
    ...
    expectedStorage = $sourcePlanSummary.expectedStorage
  ...
  $acceptance = [ordered]@{
    schemaVersion = 'radar-unified-rating-incremental-rehearsal-acceptance-9988-v01'
    accepted = $true
...
if ($null -ne $operationError) {
  Write-RadarJsonFile (Join-Path $outDir 'rehearsal-failure.json') ([ordered]@{
    schemaVersion = 'radar-unified-rating-incremental-rehearsal-failure-9988-v01'
...
Write-Host ''
Write-Host '9,988 条增量 Release 临时数据库演练已完成。' -ForegroundColor Green
