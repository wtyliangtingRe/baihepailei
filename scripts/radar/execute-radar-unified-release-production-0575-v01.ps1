...
$ExpectedFinalMigrationRows = @(
  "20260718_072813_existing_schema_baseline_v01`t1",
  "20260718_072843_stewardship_notices_v01`t1",
  "20260723_141905_current_schema_baseline_before_radar_public_v01`t2",
  "20260723_141908_radar_public_conclusions_v01`t2",
  "20260801_101546_current_schema_baseline_before_radar_public_records_v01`t1",
  "20260801_101551_radar_public_records_v01`t1",
  "20260802_030535_radar_public_ratings_v01`t1",
  "20260802_045057_radar_public_record_fact_value_text_v01`t1",
  "20260802_062015_radar_public_record_evidence_role_v01`t1"
)
...
function Assert-RadarCriticalCode([object]$AuthorizationJson) {
  ...
  foreach ($entry in @($AuthorizationJson.criticalCodeFiles)) {
    ...
    if ($bytes -ne [long]$entry.bytes -or $sha -ne [string]$entry.sha256) {
      throw "关键代码已变化：$($entry.file)"
    }
...
function Assert-RadarInitialState([object]$Snapshot, [string]$Label) {
  ...
  if ($newTables.Count -ne 0) { throw "$Label 已存在生产目标或 apply-control 表：$($newTables -join ', ')" }
}
...
function Invoke-RadarImporter(
  ...
  Invoke-RadarWithEnvironment -Variables @{
    ...
    if ($LASTEXITCODE -ne 0) { throw "Production importer $Mode 失败。" }
  }
}
...
$stage = [ordered]@{
  schemaVersion = 'radar-unified-release-production-stage-0575-v01'
...
try {
  ...
  Assert-RadarMigrationRows (Get-RadarMigrationRows $tempContainer $tempDatabase $tempUser $tempPassword) $ExpectedFinalMigrationRows 'rehearsal after migrations'

  $nonce = [Guid]::NewGuid().ToString('N')
  $app = Start-RadarPayloadApp $tempDatabaseUrl $tempDatabase 'plan' $nonce 'rehearsal-plan' $outDir
  Invoke-RadarImporter 'plan' $app.BaseUrl $tempDatabase 'plan' $nonce $email $password $releaseDirectory (Join-Path $outDir 'rehearsal-plan-import')
  ...
  Invoke-RadarImporter 'apply' $app.BaseUrl $tempDatabase 'apply' $nonce $email $password $releaseDirectory (Join-Path $outDir 'rehearsal-apply-import')
  $stage.rehearsalApplyPassed = $true
  Write-RadarJsonFile $stagePath $stage
  Invoke-RadarImporter 'verify' $app.BaseUrl $tempDatabase 'apply' $nonce $email $password $releaseDirectory (Join-Path $outDir 'rehearsal-verify-import')
  Stop-RadarProcess $app.Process
  $app = $null
  ...
  $rehearsalReceipt = Get-Content -LiteralPath (Join-Path $outDir 'rehearsal-apply-import\accepted-receipt.json') -Raw -Encoding UTF8 | ConvertFrom-Json -Depth 100
  ...
  Assert-RadarMigrationRows (Get-RadarMigrationRows $SourcePostgresContainer $SourceDatabase $SourceDatabaseUser '') $ExpectedFinalMigrationRows 'production after migrations'
  ...
  Invoke-RadarImporter 'apply' $app.BaseUrl $SourceDatabase 'apply' $nonce $email $password $releaseDirectory (Join-Path $outDir 'production-apply-import')
  $stage.productionImportApplied = $true
  Write-RadarJsonFile $stagePath $stage
  Invoke-RadarImporter 'verify' $app.BaseUrl $SourceDatabase 'apply' $nonce $email $password $releaseDirectory (Join-Path $outDir 'production-verify-import')
  Stop-RadarProcess $app.Process
  $app = $null
  $stage.productionVerifyPassed = $true
  Write-RadarJsonFile $stagePath $stage

  $productionReceipt = Get-Content -LiteralPath (Join-Path $outDir 'production-apply-import\accepted-receipt.json') -Raw -Encoding UTF8 | ConvertFrom-Json -Depth 100
  ...
  Write-RadarJsonFile (Join-Path $outDir 'production-acceptance.json') ([ordered]@{
    schemaVersion = 'radar-unified-release-production-acceptance-0575-v01'
    accepted = $true
    ...
    decision = 'accept_unified_release_production_apply_0575_v01'
  })

  Copy-Item -Path (Join-Path $outDir '*') -Destination $evidenceDir -Recurse -Force
  Remove-Item -LiteralPath (Join-Path $evidenceDir 'database-backup.dump') -Force -ErrorAction SilentlyContinue
  ...
  Stop-RadarProcess $(if ($app) { $app.Process } else { $null })
  if ($tempCreated) { & docker rm -f $tempContainer 2>$null | Out-Null }
  if ($markerCreated -and -not $productionCompleted) {
    try {
      Set-RadarApplyMarkerState $SourcePostgresContainer $SourceDatabase $SourceDatabaseUser '' 'failed' 'production-failure' $outDir
    } catch {}
  }
...
if ($null -ne $operationError) {
  Write-RadarJsonFile (Join-Path $outDir 'production-failure.json') ([ordered]@{
    schemaVersion = 'radar-unified-release-production-failure-0575-v01'
