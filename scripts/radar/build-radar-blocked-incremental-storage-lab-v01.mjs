...
export const VERSION = 'radar-blocked-incremental-storage-lab-v0.2'
export const EXPECTED_ASSEMBLY_ZIP_SHA256 = 'd41fb41951e35e8dc0c2cdde1fb904968af3df5d3564bddb8a13a3fb002c14c2'
export const EXPECTED_ROWS = 683
export const EXPECTED_BASELINE_ROWS = 9000
export const EXPECTED_FINAL_ROWS = 9683
export const EXPECTED_INCREMENTAL_GRADES = canonical({ A: 51, B: 423, C: 146, D: 40, E: 15, F: 8 })
export const EXPECTED_FINAL_GRADES = canonical({ S: 8, A: 318, B: 1098, C: 1014, D: 7072, E: 153, F: 20 })
export const INPUT_PATH_IN_CONTAINER = '/tmp/radar-blocked-incremental-storage-ready.jsonl'
...
export function assertManifest(directory) {
  ...
  if (actual.length !== expected.size || actual.some((item) => !expected.has(item))) {
    throw new Error('Assembly manifest coverage mismatch.')
  }
  return manifest
}
...
function inputTableSql() {
  return `CREATE TEMP TABLE radar_blocked_incremental_input_raw (line text NOT NULL) ON COMMIT DROP;\nCOPY radar_blocked_incremental_input_raw(line) FROM '${INPUT_PATH_IN_CONTAINER}' WITH (FORMAT csv, DELIMITER E'\\x1f', QUOTE E'\\x1e', ESCAPE E'\\x1e');`
}

function inputCte() {
  return `WITH input AS (\n  SELECT (line::jsonb)->'publicRecord' AS r\n  FROM radar_blocked_incremental_input_raw\n)`
}
...
export function buildSqlBundle(validated) {
  ...
  const acceptanceChecks = [
    ...
    checkLine('input_rows', `(SELECT count(*) FROM radar_public p JOIN radar_blocked_incremental_input_raw raw ON p.publication_key=(raw.line::jsonb#>>'{publicRecord,publicationKey}'))`, String(EXPECTED_ROWS)),
    checkLine('baseline_rows_retained', `(SELECT count(*) FROM radar_public p WHERE NOT EXISTS (SELECT 1 FROM radar_blocked_incremental_input_raw raw WHERE p.publication_key=(raw.line::jsonb#>>'{publicRecord,publicationKey}')))`, String(EXPECTED_BASELINE_ROWS)),
    ...
    checkLine('input_review_reason_rows', `(SELECT count(*) FROM radar_public_review_reasons r JOIN radar_public p ON p.id=r.parent_id JOIN radar_blocked_incremental_input_raw raw ON p.publication_key=(raw.line::jsonb#>>'{publicRecord,publicationKey}'))`, String(reviewReasonRows)),
    checkLine('input_matched_rule_rows', `(SELECT count(*) FROM radar_public_radar_assessment_matched_rules r JOIN radar_public p ON p.id=r._parent_id JOIN radar_blocked_incremental_input_raw raw ON p.publication_key=(raw.line::jsonb#>>'{publicRecord,publicationKey}'))`, String(matchedRuleRows)),
    checkLine('input_contradiction_rows', `(SELECT count(*) FROM radar_public_radar_assessment_contradictions r JOIN radar_public p ON p.id=r._parent_id JOIN radar_blocked_incremental_input_raw raw ON p.publication_key=(raw.line::jsonb#>>'{publicRecord,publicationKey}'))`, String(contradictionRows)),
    checkLine('input_publication_key_set_md5', `(SELECT md5(string_agg(p.publication_key, E'\\n' ORDER BY p.publication_key)) FROM radar_public p JOIN radar_blocked_incremental_input_raw raw ON p.publication_key=(raw.line::jsonb#>>'{publicRecord,publicationKey}'))`, sqlString(md5Sorted(publicationKeys))),
    checkLine('input_work_id_set_md5', `(SELECT md5(string_agg(p.work_id::text, E'\\n' ORDER BY p.work_id::text)) FROM radar_public p JOIN radar_blocked_incremental_input_raw raw ON p.publication_key=(raw.line::jsonb#>>'{publicRecord,publicationKey}'))`, sqlString(md5Sorted(workIds))),
    checkLine('input_conclusion_hash_set_md5', `(SELECT md5(string_agg(p.conclusion_sha256, E'\\n' ORDER BY p.conclusion_sha256)) FROM radar_public p JOIN radar_blocked_incremental_input_raw raw ON p.publication_key=(raw.line::jsonb#>>'{publicRecord,publicationKey}'))`, sqlString(md5Sorted(conclusionHashes))),
    checkLine('input_single_published_at', `(SELECT count(DISTINCT p.published_at) FROM radar_public p JOIN radar_blocked_incremental_input_raw raw ON p.publication_key=(raw.line::jsonb#>>'{publicRecord,publicationKey}'))`, '1'),
...
export function buildOutputs({ assemblyDir, assemblyZipSha256, outDir }) {
  if (val(assemblyZipSha256).toLowerCase() !== EXPECTED_ASSEMBLY_ZIP_SHA256) {
    throw new Error('Assembly dry-run ZIP SHA-256 mismatch.')
  }
  assertManifest(assemblyDir)
  const summary = readJson(path.join(assemblyDir, 'radar-blocked-assembly-dryrun-summary.json'))
  const validation = readJson(path.join(assemblyDir, 'assembly-dryrun-validation.json'))
  if (summary?.rows !== EXPECTED_ROWS || summary?.storageNormalized !== EXPECTED_ROWS || summary?.blocked !== 0 || list(summary?.globalBlockers).length) {
    throw new Error('Assembly summary is not ready for storage lab planning.')
  }
  if (summary?.sources?.productionPublicBaseline !== EXPECTED_BASELINE_ROWS || summary?.readyForNextStorageLabPlanning !== true) {
    throw new Error('Assembly summary baseline or next-step gate mismatch.')
  }
  if (validation?.passed !== true || validation?.storageChangedFieldsOnly !== true || list(validation?.globalBlockers).length) {
    throw new Error('Assembly validation did not pass.')
  }
  ...
  const outputs = {
    ...
    summary: path.join(outDir, 'radar-blocked-incremental-storage-lab-plan-summary.json'),
  }
