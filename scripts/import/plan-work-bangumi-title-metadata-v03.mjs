...
const VERSION = 'work-bangumi-title-metadata-plan-v0.3'
...
function unwrapCandidates(row) {
  const out = []
  const push = (value, pathName) => {
    if (!value || typeof value !== 'object') return
    if (Array.isArray(value)) {
      value.forEach((item, index) => push(item, `${pathName}[${index}]`))
      return
    }
    if (isSubjectLike(value)) out.push({ record: value, pathName })
    for (const key of ['raw', 'subject', 'candidate', 'data', 'item']) {
      if (value[key]) push(value[key], `${pathName}.${key}`)
    }
  }
  push(row, '$')
  return out
...
function localizedTitles(record) {
  const rows = []
  ...
  for (const row of infoboxRows(record, /中文名|中文名/i)) add(row?.value, 'zh-Hans', 'localized', 'Bangumi infobox 中文名')
...
function normalizeRecord(file, wrapper, record, pathName) {
  ...
  if (!id) return null
  const titles = localizedTitles(record)
  ...
  return {
    ...
    titles: unique(titles.map((row) => row.title)),
    localizedTitles: titles,
    aliases: unique(titles.filter((row) => ['alias', 'romanized'].includes(row.kind)).map((row) => row.title)),
...
function titleKeysOfWork(doc) {
  return unique([doc.title, doc.originalTitle, currentLocalizedTitles(doc), currentAliases(doc)]).map((x) => x.toLowerCase()).filter((x) => x.length >= 2 && !isMojibake(x))
}

function existingTitleKeys(doc) {
  return new Set(titleKeysOfWork(doc))
}
...
function planForWork(doc, records, matchMode) {
  const existing = existingTitleKeys(doc)
  ...
  for (const record of records) {
    for (const row of record.localizedTitles) {
      if (!existing.has(row.title.toLowerCase())) addLocalizedTitles.push({ ...row, note: `${row.note}; match=${matchMode}` })
    }
    for (const alias of record.aliases) {
      if (!existing.has(alias.toLowerCase())) addAliases.push({ value: alias })
    }
  ...
  const additions = {
    ...
    candidateSources: addCandidateSources,
  ...
  return {
    ...
    matchMode,
    keys: workKeys(doc),
    matchedRecords: records.map((x) => ({ source: 'bangumi', externalId: x.externalId, file: x.file, pathName: x.pathName, titles: x.titles.slice(0, 8), date: x.date, hasSummary: Boolean(x.summary), skippedMojibake: x.skippedMojibake })),
    additions,
    changedFields,
  }
}
...
async function main() {
  ...
  const scanRoots = (String(args.roots || '').split(';').map(clean).filter(Boolean).length ? String(args.roots).split(';').map(clean).filter(Boolean) : DEFAULT_ROOTS).filter((root) => fs.existsSync(root))
  ...
  for (const file of files) {
    ...
    for await (const wrapper of readRows(file)) {
      ...
      for (const { record, pathName } of unwrapCandidates(wrapper)) {
        unwrapped += 1
        const normalized = normalizeRecord(file, wrapper, record, pathName)
  ...
  for (const doc of works.docs) {
    const keyMatches = unique(workKeys(doc)).flatMap((key) => byId.get(key) || [])
    if (keyMatches.length) {
      const plan = planForWork(doc, keyMatches, 'external_id')
      if (plan.changedFields.length) plans.push(plan)
      continue
    }
    const titleMatches = unique(titleKeysOfWork(doc).flatMap((key) => byTitle.get(key) || []))
    if (titleMatches.length) {
      const ids = unique(titleMatches.map((x) => x.externalId))
      const plan = planForWork(doc, titleMatches, ids.length === 1 ? 'title_exact' : 'title_exact_multi')
      if (ids.length === 1) {
        if (plan.changedFields.length) plans.push(plan)
      } else {
  ...
  const outputs = {
    plans: path.join(outDir, 'work-bangumi-title-metadata-v03-plans.jsonl'),
    deferred: path.join(outDir, 'work-bangumi-title-metadata-v03-deferred.jsonl'),
    unmatched: path.join(outDir, 'work-bangumi-title-metadata-v03-unmatched.jsonl'),
    files: path.join(outDir, 'work-bangumi-title-metadata-v03-files.json'),
    summary: path.join(outDir, 'work-bangumi-title-metadata-v03-summary.json'),
    json: path.join(outDir, 'work-bangumi-title-metadata-v03.json'),
  }
  const summary = {
  ...
  fs.writeFileSync(outputs.deferred, deferred.map((x) => JSON.stringify(x)).join('\n') + (deferred.length ? '\n' : ''), 'utf8')
  fs.writeFileSync(outputs.unmatched, unmatched.map((x) => JSON.stringify(x)).join('\n') + (unmatched.length ? '\n' : ''), 'utf8')
