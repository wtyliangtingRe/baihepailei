...
const VERSION = 'vndb-lesbian-sex-only-create-works-plan-v0.1'
const DEFAULT_INPUT = 'data_local/staging/vndb-yuri-create-candidates/vndb-yuri-create-candidates-v03-lesbian-sex-only-review.jsonl'
const DEFAULT_OUT_DIR = 'data_local/staging/vndb-lesbian-sex-only-create-works'
const IMPORT_BATCH = 'vndb-lesbian-sex-only-adult-v01'
const ADULT_NOTE = 'contentRating=erotica; contentVisibility=adult; adultOrMarkedContent=true; sourcePolicy=VNDB Lesbian Sex only adult draft; hidden from ordinary mode by derived content visibility until manually reviewed.'
...
function adultCandidateSources(row) {
  const rows = list(row?.createCandidatePreview?.candidateSources || row?.fieldAdditions?.candidateSources)
  const out = rows.map((item) => {
    const note = cleanLine(item?.note)
    return {
      ...
      note: [note, ADULT_NOTE].filter(Boolean).join('; '),
    }
  }).filter((item) => item.source || item.externalId || item.url || item.note)
  if (!out.length) {
    out.push({ source: 'vndb', label: 'VNDB', externalId: val(row?.vndbId), url: val(row?.vndbUrl), note: ADULT_NOTE })
  }
  return out
}

function matchedTags(row) { return list(row?.yuriCreateCandidate?.matchedTags) }
...
function sensitiveReasons(row) {
  return uniqueBy(list(row?.yuriCreateCandidateV2?.sensitiveReasons || row?.yuriCreateCandidate?.sensitiveReasons).map(val).filter(Boolean), (item) => item)
}
...
function buildPlan(row) {
  ...
  const reasons = sensitiveReasons(row)

  if (!vndbId) blockers.push('missing_vndb_id')
  if (!title) blockers.push('missing_title')
  if (statusV2 !== 'lesbian_sex_only_review') blockers.push('not_lesbian_sex_only_review')
  if (statusV3 !== 'lesbian_sex_only_review') blockers.push('not_v03_lesbian_sex_only_review')
  if (g82 < 1) blockers.push('missing_g82_lesbian_sex_tag_rating')
  ...
  const payload = {
    ...
    format: 'visual_novel',
    yuriCandidateScore: Math.max(0.05, Math.min(0.7, g82 / 4)),
    isLiteVisible: true,
    isFullVisible: true,
    externalIds: externalIdsOf(row),
    sourceLinks: sourceLinks(row),
    candidateSources: adultCandidateSources(row),
    searchText: searchTextOf(row, title, originalTitle),
    evidenceNote: [
      'Adult-content catalog draft generated from VNDB Lesbian Sex tag metadata only.',
      'This is a source marker, not a reviewed yuri relationship judgement.',
      'Human review and evidence assessment have not been applied.',
      ADULT_NOTE,
      `VNDB ID: ${vndbId}`,
      `VNDB Lesbian Sex tag rating: ${g82}`,
      reasons.length ? `Sensitive/adult reasons carried from planner: ${reasons.join(', ')}` : '',
    ].filter(Boolean).join('\n'),
    status: 'draft',
  }
  ...
  return {
    ...
    vndbId,
    title,
    action: 'create_vndb_lesbian_sex_only_adult_work_candidate',
    createType: 'adult_lesbian_sex_only',
...
function main() {
  ...
  const summary = {
    ...
    bySensitiveReason: countBy(plans.flatMap((row) => row.sourceRow.sensitiveReasons || []), (item) => item),
    outputs: {
      rows: `${outDir}/vndb-lesbian-sex-only-create-works-v01.rows.jsonl`,
      ready: `${outDir}/vndb-lesbian-sex-only-create-works-v01-ready.jsonl`,
      blocked: `${outDir}/vndb-lesbian-sex-only-create-works-v01-blocked.jsonl`,
      sample: `${outDir}/vndb-lesbian-sex-only-create-works-v01-sample.jsonl`,
      summary: `${outDir}/vndb-lesbian-sex-only-create-works-v01-summary.json`,
    },
    safety: {
      payloadRead: false,
      payloadWrite: false,
      ...
      inputMustBeV03LesbianSexOnlyReview: true,
      requiresG82LesbianSexTag: true,
      marksAdultInCandidateSourceNotes: true,
      ordinaryModeCanHideByDerivedContentVisibility: true,
