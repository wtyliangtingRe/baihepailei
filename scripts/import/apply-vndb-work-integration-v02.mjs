...
const VERSION = 'vndb-work-integration-apply-preflight-v0.2'
...
function main() {
  ...
  fs.writeFileSync(preflightSummary, JSON.stringify({
    ...
    note: 'Sensitive/adult/suggestive/violent VNDB rows are excluded from base apply until a dedicated content-visibility flow can write advisory fields safely.',
  }, null, 2), 'utf8')
  console.log(JSON.stringify({ ok: true, preflight: { rowsRead: rows.length, rowsSafeForBaseApply: safe.length, rowsBlockedByPreflight: blocked.length, preflightSummary } }, null, 2))
