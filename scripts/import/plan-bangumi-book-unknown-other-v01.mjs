...
for (const row of rows) {
  const works = Array.isArray(row.matchedWorks) ? row.matchedWorks : []
  const unknownWorks = works.filter((w) => clean(w.mediaGroup || 'unknown') === 'unknown' || clean(w.mediaType || 'unknown') === 'unknown')
  if (!unknownWorks.length) { skipped.push({ bangumiId: row.bangumiId, reason: 'no_unknown_work' }); continue }
  for (const work of unknownWorks) plans.push({ workId: work.id, slug: work.slug, title: work.title, bangumiId: row.bangumiId, targetMediaGroup: 'other', targetMediaType: 'other', reason: 'bangumi_book_unknown_unresolved', sourceStatus: row.status })
}
...
const summary = { version: 'bangumi-book-unknown-other-plan-v0.1', inputFile: input, rowsRead: rows.length, plannedWorks: plans.length, skippedRows: skipped.length, byTarget: count(plans, 'targetMediaGroup'), bySkippedReason: count(skipped, 'reason'), safety: { readOnly: true, payloadWrite: false, dataChanged: false }, outputs }
