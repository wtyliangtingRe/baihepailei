...
const VERSION = 'steam-create-candidates-apply-v0.3'
...
function cleanLine(value) {
  return val(value)
    .normalize('NFKC')
    .replace(/[\r\n\t]+/gu, ' ')
    .replace(new RegExp(TITLE_SEPARATOR_PATTERN, 'gu'), ' ')
    .trim()
}
...
function sourceLinkRows(doc) {
  return list(doc?.sourceLinks)
    .map((item) => ({ label: cleanLine(item?.label), url: normalizeUrl(item?.url) }))
...
function steamIdsOfWork(work) {
  const ids = []
  for (const item of [...sourceLinkRows(work), ...candidateSourceRows(work)]) {
    const hit = item.url?.match(/store\.steampowered\.com\/app\/(\d+)/iu)?.[1]
    if (hit) ids.push(hit)
...
function buildIndexes(works) {
  const steam = new Map(), slug = new Map(), siteId = new Map(), title = new Map()
  for (const work of works) {
    for (const id of steamIdsOfWork(work)) steam.set(id, work)
  ...
  return { steam, slug, siteId, title }
}
...
function hasSensitiveSteamWarning(plan) {
  const text = [
    plan?.warnings,
    plan?.raw?.required_age,
    plan?.raw?.content_descriptors?.notes,
    JSON.stringify(plan?.raw?.ratings || {}),
    JSON.stringify(plan?.createCandidatePreview?.candidateSources || []),
  ].flat().map(val).join('\n')
  return /adult|sexual|nudity|erotic|porn|sex|sexo|sexuelle|explicit|18|2147483647|banned/iu.test(text)
}
function safeReviewReasons(sourceReasons, sensitive) {
  const reasons = [
    ...list(sourceReasons).filter((item) => ALLOWED_REVIEW_REASONS.has(item)),
    'manual_review',
    sensitive ? 'source_conflict' : 'multi_source_or_variant',
  ]
  return [...new Set(reasons)]
}
function adultMarkerParts(sensitive) { return sensitive ? [ADULT_MARKER_NOTE] : [] }
function markerHaystack(payload) {
  return [
    payload?.evidenceNote,
    payload?.sourceConflictNotes,
    payload?.searchText,
    ...list(payload?.candidateSources).map((item) => item?.note),
...
function buildPayload(plan) {
  ...
  const sensitive = hasSensitiveSteamWarning(plan)
  ...
  const candidateSources = candidateSourceRows({ candidateSources: source.candidateSources || plan.createCandidatePreview?.candidateSources })
    .map((item) => ({
      ...item,
      note: cleanLine([
        item.note,
        ...markerParts,
        'sourcePolicy=Steam manual review create; metadata-only; human review required',
      ].filter(Boolean).join('; ')),
    }))
  const evidenceNote = cleanLine([
    source.evidenceNote,
    ...markerParts,
    'Created from Steam create candidate. Draft status; human review required before final review. No dates/images/creators/tags/summary/externalIds written by this workflow.',
  ].filter(Boolean).join(' '))
  const sourceConflictNotes = cleanLine([
    source.sourceConflictNotes,
    sensitive
      ? 'Steam candidate has adult/sensitive rating or descriptor signals; ordinary mode hides it through derived content visibility until user switches to all works.'
      : 'Steam candidate created from strong source signals; pending human review.',
    ...markerParts,
  ].filter(Boolean).join(' '))
  return {
    title,
    ...
    reviewReasons: safeReviewReasons(source.reviewReasons, sensitive),
    sourceConflictNotes,
    evidenceStrength: source.evidenceStrength || 'unassessed',
    ratingNotice: source.ratingNotice || 'ai_synthesized_pending_review',
    importBatch: 'steam-create-candidates-v01',
    chosenBaseSource: 'steam',
    originalTitle: cleanLine(source.originalTitle || title),
    mediaGroup: source.mediaGroup || 'game',
    mediaType: source.mediaType || 'game',
    format: source.format || 'pc_game',
    yuriCandidateScore: Number(source.yuriCandidateScore || 0.5),
    sourceLinks,
    candidateSources,
...
function validatePlan(plan, payload) {
  const blockers = []
  const sensitive = hasSensitiveSteamWarning(plan)
  if (val(plan?.action) !== 'steam_create_candidate_preview') blockers.push('unexpected_action')
  if (plan?.planStatus !== 'create_candidate_review') blockers.push('not_create_candidate_review')
  ...
  if (payload.reviewStatus !== 'pending') blockers.push('not_pending_review')
  if (payload.isLiteVisible !== true) blockers.push('not_lite_visible_for_scope_toggle')
  if (payload.isFullVisible !== true) blockers.push('not_full_visible_for_scope_toggle')
  if (!list(payload.sourceLinks).some((item) => /store\.steampowered\.com\/app\//iu.test(item.url))) blockers.push('missing_steam_source_link')
  if (!list(payload.candidateSources).some((item) => item.source === 'steam' && item.externalId === val(plan.steamAppId))) blockers.push('missing_matching_steam_candidate_source')
  for (const reason of list(payload.reviewReasons)) if (!ALLOWED_REVIEW_REASONS.has(reason)) blockers.push(`invalid_review_reason:${reason}`)
  if (sensitive) {
    const markers = markerHaystack(payload)
    if (!markers.includes('contentrating=erotica')) blockers.push('missing_adult_content_rating_note')
    if (!markers.includes('contentvisibility=adult')) blockers.push('missing_adult_visibility_note')
    if (!markers.includes('adultormarkedcontent=true')) blockers.push('missing_adult_marker_note')
  }
  for (const key of ['firstPublishedAt', 'firstReleasedAt', 'publishedAt', 'creators', 'creatorCredits', 'organizations', 'tags', 'summary', 'externalIds']) {
    if (key in payload) blockers.push(`non_writable_create_field:${key}`)
  }
  for (const value of [payload.title, payload.originalTitle, payload.slug, ...splitSearchText(payload.searchText)]) {
    if (/\[object Object\]/u.test(value)) blockers.push('object_object_artifact')
    if (suspiciousTitleSpaceScore(value) > 0) blockers.push('suspicious_title_spacing_after_sanitize')
  }
  return [...new Set(blockers)]
}
function duplicateStatus(payload, plan, indexes) {
  const blockers = []
  const steamAppId = val(plan.steamAppId)
  if (steamAppId && indexes.steam.has(steamAppId)) blockers.push('existing_steam_appid')
  if (indexes.slug.has(val(payload.slug).toLowerCase())) blockers.push('existing_slug')
  if (indexes.siteId.has(val(payload.siteId).toUpperCase())) blockers.push('existing_site_id')
...
function updateIndexes(payload, doc, plan, indexes) {
  const steamAppId = val(plan.steamAppId)
  if (steamAppId) indexes.steam.set(steamAppId, doc)
  if (payload.slug) indexes.slug.set(val(payload.slug).toLowerCase(), doc)
  if (payload.siteId) indexes.siteId.set(val(payload.siteId).toUpperCase(), doc)
  for (const key of exactTitleSet(payload)) {
...
async function main() {
  ...
  for (const plan of plans) {
    ...
    const row = {
      key: val(plan.key),
      steamAppId: val(plan.steamAppId),
      title: payload.title,
      mode: apply ? 'apply' : 'dry-run',
      status: '',
      blockers: validatePlan(plan, payload),
      createdWorkId: '',
      slug: payload.slug,
      siteId: payload.siteId,
      sensitive: hasSensitiveSteamWarning(plan),
      adultMarkerApplied: markerHaystack(payload).includes('contentvisibility=adult'),
    }
    try {
  ...
  const outputs = {
    rows: `${outDir}/steam-create-candidates-apply-v01.rows.jsonl`,
    wouldCreate: `${outDir}/steam-create-candidates-apply-v01-would-create.jsonl`,
    created: `${outDir}/steam-create-candidates-apply-v01-created.jsonl`,
    blocked: `${outDir}/steam-create-candidates-apply-v01-blocked.jsonl`,
    failed: `${outDir}/steam-create-candidates-apply-v01-failed.jsonl`,
    summary: `${outDir}/steam-create-candidates-apply-v01-summary.json`,
  }
  const summary = {
    ...
    wouldCreate: rows.filter((r) => r.status === 'would_create').length,
    created: rows.filter((r) => r.status === 'created').length,
    alreadyExists: rows.filter((r) => r.status === 'already_exists').length,
    blocked: rows.filter((r) => r.status === 'blocked').length,
    failed: rows.filter((r) => r.status === 'failed').length,
    sensitiveRows: rows.filter((r) => r.sensitive).length,
    adultMarkerAppliedRows: rows.filter((r) => r.adultMarkerApplied).length,
    byStatus: countBy(rows, 'status'),
    byBlocker: countBy(rows.flatMap((r) => r.blockers), (x) => x),
    outputs,
    safety: {
      ...
      deletesWorks: false,
      scopeToggleCompatible: true,
      sensitiveRowsCarryAdultMarkers: true,
      adultMarkerNote: ADULT_MARKER_NOTE,
      inputDefaultIsP1BangumiAdultReview: input === DEFAULT_INPUT,
      usesOnlyAllowedReviewReasonEnums: true,
      blocksDuplicates: true,
      doesNotWriteDates: true,
      doesNotDownloadImages: true,
      doesNotWriteCreatorsOrTags: true,
      doesNotWriteSummary: true,
      doesNotWriteExternalIds: true,
