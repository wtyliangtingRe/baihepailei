...
const VERSION = 'steam-create-candidates-apply-v0.2'
...
function buildPayload(plan) {
  ...
  const candidateSources = candidateSourceRows({ candidateSources: source.candidateSources || plan.createCandidatePreview?.candidateSources }).map((item) => ({ ...item, note: cleanLine([item.note, 'sourcePolicy=Steam manual/adult-review draft create; metadata-only; human review required'].filter(Boolean).join('; ')) }))
  const evidenceNote = cleanLine([source.evidenceNote, 'Created from Steam P1 Bangumi high/medium adult-review candidate. Draft only; human review required before publish. No dates/images/creators/tags/summary/externalIds written by this workflow.'].filter(Boolean).join(' '))
  const sourceConflictNotes = cleanLine([source.sourceConflictNotes, sensitive ? 'Steam candidate has adult/sensitive rating or descriptor signals; kept as draft pending human review.' : 'Steam P1 candidate created from Bangumi high/medium relation; pending human review.'].filter(Boolean).join(' '))
  return {
    title,
    slug,
    siteId,
...
function validatePlan(plan, payload) { const blockers = []; if (val(plan?.action) !== 'steam_create_candidate_preview') blockers.push('unexpected_action'); if (plan?.planStatus !== 'create_candidate_review') blockers.push('not_create_candidate_review'); if (!val(plan?.steamAppId)) blockers.push('missing_steam_appid'); if (!val(payload.title)) blockers.push('missing_title'); if (!val(payload.slug)) blockers.push('missing_slug'); if (!val(payload.siteId)) blockers.push('missing_site_id'); if (payload.status !== 'draft') blockers.push('not_draft_status'); if (payload.reviewStatus !== 'pending') blockers.push('not_pending_review'); if (!list(payload.sourceLinks).some((item) => /store\.steampowered\.com\/app\//iu.test(item.url))) blockers.push('missing_steam_source_link'); if (!list(payload.candidateSources).some((item) => item.source === 'steam' && item.externalId === val(plan.steamAppId))) blockers.push('missing_matching_steam_candidate_source'); for (const reason of list(payload.reviewReasons)) if (!ALLOWED_REVIEW_REASONS.has(reason)) blockers.push(`invalid_review_reason:${reason}`); for (const key of ['firstPublishedAt', 'firstReleasedAt', 'publishedAt', 'creators', 'creatorCredits', 'organizations', 'tags', 'summary', 'externalIds']) if (key in payload) blockers.push(`non_writable_create_field:${key}`); for (const value of [payload.title, payload.originalTitle, payload.slug, ...splitSearchText(payload.searchText)]) { if (/\[object Object\]/u.test(value)) blockers.push('object_object_artifact'); if (suspiciousTitleSpaceScore(value) > 0) blockers.push('suspicious_title_spacing_after_sanitize') } return [...new Set(blockers)] }
...
async function main() {
  ...
  for (const plan of plans) {
    ...
    const row = { key: val(plan.key), steamAppId: val(plan.steamAppId), title: payload.title, mode: apply ? 'apply' : 'dry-run', status: '', blockers: validatePlan(plan, payload), createdWorkId: '', slug: payload.slug, siteId: payload.siteId }
    try {
  ...
  const summary = { generatedAt: new Date().toISOString(), version: VERSION, mode: apply ? 'apply' : 'dry-run', payloadBaseUrl: base, input, planRowsRead: allPlans.length, planRowsProcessed: plans.length, wouldCreate: rows.filter((r) => r.status === 'would_create').length, created: rows.filter((r) => r.status === 'created').length, alreadyExists: rows.filter((r) => r.status === 'already_exists').length, blocked: rows.filter((r) => r.status === 'blocked').length, failed: rows.filter((r) => r.status === 'failed').length, byStatus: countBy(rows, 'status'), byBlocker: countBy(rows.flatMap((r) => r.blockers), (x) => x), outputs, safety: { applyRequested: apply, confirmMatched, payloadRead: true, payloadWrite: apply, payloadPostRequests, directPostgresqlWrite: false, createsOnlyDraftWorks: true, deletesWorks: false, inputDefaultIsP1BangumiAdultReview: input === DEFAULT_INPUT, usesOnlyAllowedReviewReasonEnums: true, blocksDuplicates: true, doesNotWriteDates: true, doesNotDownloadImages: true, doesNotWriteCreatorsOrTags: true, doesNotWriteSummary: true, doesNotWriteExternalIds: true, confirmToken: CONFIRM } }
