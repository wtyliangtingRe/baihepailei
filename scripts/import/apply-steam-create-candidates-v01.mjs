...
const VERSION = 'steam-create-candidates-apply-v0.1'
...
function buildPayload(plan) {
  ...
  const reviewReasons = [...new Set([...list(source.reviewReasons), 'manual_review', 'steam_create_candidate', hasSensitiveSteamWarning(plan) ? 'adult_or_sensitive_review' : 'external_source_review'])]
  const evidenceNote = cleanLine([source.evidenceNote, 'Created from Steam P1 Bangumi high/medium adult-review candidate. Draft only; human review required before publish. No dates/images/creators/tags/summary written by this workflow.'].filter(Boolean).join(' '))
  return {
    title,
    slug,
    siteId,
...
async function main() {
  ...
  writeJsonl(outputs.blocked, rows.filter((r) => r.status === 'blocked' || r.status === 'already_exists' || r.status === 'failed'))
