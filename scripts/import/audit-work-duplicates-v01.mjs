...
const VERSION = 'work-duplicates-audit-v0.1'
...
function sourceKeys(work) { const out = []
...
function titleKeys(work) { return [...new Set([work.title, work.originalTitle, ...list(work.aliases).map((x) => x?.value), ...searchLines(work.searchText).filter((line) => line.length <= 120 && !/^https?:\/\//iu.test(line))].map(normalizedTitle).filter((key) => key.length >= 2))] }
...
function workSummary(work) { return { id: work.id, title: work.title, slug: work.slug, siteId: work.siteId, status: work.status, rank: work.rank, mediaGroup: work.mediaGroup, mediaType: work.mediaType, format: work.format, hasBangumi: hasBangumi(work), sourceKeyCount: sourceKeys(work).length, sourceKeys: sourceKeys(work).slice(0, 30), createdAt: work.createdAt, updatedAt: work.updatedAt } }
...
function confidenceFor(kind, works, key) { if (kind === 'source_key') return 'high'; const mediaTypes = new Set(works.map((w) => val(w.mediaType || 'unknown'))); const mediaGroups = new Set(works.map((w) => val(w.mediaGroup || 'unknown'))); if (kind === 'title_media' && mediaTypes.size === 1 && mediaGroups.size === 1) return 'medium'; if (key && key.length >= 6) return 'low'; return 'review' }
...
async function main() { const args = parseArgs(process.argv.slice(2)); const base = String(args.url || process.env.NEXT_PUBLIC_SERVER_URL || 'http://localhost:3000').replace(/\/+$/u, ''); const outDir = String(args['out-dir'] || DEFAULT_OUT_DIR); const token = await login(base); const works = await fetchAllWorks(base, token); const bySource = new Map(); const byTitleMedia = new Map(); const byTitleOnly = new Map(); for (const work of works) { for (const key of sourceKeys(work)) addGroup(bySource, key, work); const titleKey = normalizedTitle(work.title || work.originalTitle || ''); if (titleKey) { addGroup(byTitleMedia, `${work.mediaGroup || 'unknown'}|${work.mediaType || 'unknown'}|${titleKey}`, work); addGroup(byTitleOnly, titleKey, work) } }
  const rows = [];
  for (const [key, group] of bySource) if (group.length > 1) rows.push(groupRow('source_key', key, group));
  for (const [key, group] of byTitleMedia) if (group.length > 1) rows.push(groupRow('title_media', key, group));
  for (const [key, group] of byTitleOnly) if (group.length > 1) rows.push(groupRow('title_only', key, group));
  const deduped = [];
  const seen = new Set();
  for (const row of rows.sort((a, b) => ({ high: 0, medium: 1, low: 2, review: 3 }[a.confidence] - { high: 0, medium: 1, low: 2, review: 3 }[b.confidence] || b.duplicateCount - a.duplicateCount)) {
