...
const VERSION = 'vndb-lesbian-sex-only-create-works-apply-v0.1'
const DEFAULT_INPUT = 'data_local/staging/vndb-lesbian-sex-only-create-works/vndb-lesbian-sex-only-create-works-v01-ready.jsonl'
const DEFAULT_OUT_DIR = 'data_local/staging/vndb-lesbian-sex-only-create-works'
const CONFIRM = 'apply-vndb-lesbian-sex-only-create-works-v01'
const ALLOWED_ACTIONS = new Set(['create_vndb_lesbian_sex_only_adult_work_candidate'])
...
function titleValidationValues(plan) {
  ...
  return [payload.title, payload.originalTitle, ...splitSearchText(payload.searchText), ...list(plan?.sourceRow?.titleCandidates)].map(cleanLine).filter(Boolean)
}
...
function exactTitleSet(work) {
  return new Set([work?.title, work?.originalTitle, ...splitSearchText(work?.searchText)].map(normalizeText).filter(Boolean))
}
...
function notesHaystack(payload) {
  return list(payload?.candidateSources).map((item) => cleanLine(item?.note).toLowerCase()).join('\n')
}
function validatePlan(plan) {
  const blockers = []
  const payload = plan?.payload || {}
  const vndbId = val(plan?.vndbId || payload?.externalIds?.vndbId).toLowerCase()
  const statusV2 = val(plan?.sourceRow?.yuriCreateCandidateV2?.status)
  const statusV3 = val(plan?.sourceRow?.yuriCreateCandidateV3?.status)
  const haystack = notesHaystack(payload)
  ...
  if (plan?.createType !== 'adult_lesbian_sex_only') blockers.push('unexpected_create_type')
  ...
  if (!/contentrating=erotica/u.test(haystack)) blockers.push('missing_adult_content_rating_note')
  if (!/contentvisibility=adult/u.test(haystack)) blockers.push('missing_adult_visibility_note')
  if (!/adultormarkedcontent=true/u.test(haystack)) blockers.push('missing_adult_marker_note')

  for (const item of titleValidationValues(plan)) {
...
async function main() {
  ...
  const outputs = {
    rows: `${outDir}/vndb-lesbian-sex-only-create-works-apply-v01.rows.jsonl`,
    wouldCreate: `${outDir}/vndb-lesbian-sex-only-create-works-apply-v01-would-create.jsonl`,
    created: `${outDir}/vndb-lesbian-sex-only-create-works-apply-v01-created.jsonl`,
    blocked: `${outDir}/vndb-lesbian-sex-only-create-works-apply-v01-blocked.jsonl`,
    summary: `${outDir}/vndb-lesbian-sex-only-create-works-apply-v01-summary.json`,
  }
  const summary = {
    ...
    safety: {
      ...
      inputMustBeVndbLesbianSexOnlyReadyPlan: true,
      blocksDuplicateVndbQidSlugSiteId: true,
      blocksExactTitleMatches: true,
      requiresG82LesbianSexTagPlan: true,
      requiresAdultContentMarkerNotes: true,
