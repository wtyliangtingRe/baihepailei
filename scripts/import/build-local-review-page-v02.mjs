...
const OUT = 'data_local/staging/review/local-review-page-v02.html'
...
<head>
<meta charset="utf-8">
<title>百合排雷本地 Review Queue v0.3</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
:root {
  color-scheme: light dark;
  --bg:#f7f7fb; --panel:#fff; --text:#111827; --muted:#6b7280; --line:#e5e7eb;
  --p1:#fee2e2; --p2:#fef3c7; --p3:#dbeafe; --chip:#eef2ff; --active:#c7d2fe;
  --mono:ui-monospace,SFMono-Regular,Menlo,Consolas,"Liberation Mono",monospace;
  --sans:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;
}
@media (prefers-color-scheme: dark) {
  :root {
    --bg:#0f172a; --panel:#111827; --text:#e5e7eb; --muted:#9ca3af; --line:#374151;
    --p1:#4c1d1d; --p2:#463512; --p3:#172554; --chip:#1e1b4b; --active:#3730a3;
  }
}
...
.card, .toolbar, .table-wrap, details { background:var(--panel); border:1px solid var(--line); border-radius:14px; }
...
.table-wrap { overflow:auto; max-height:calc(100vh - 280px); min-height:380px; }
...
<body>
<header>
  <h1>百合排雷本地 Review Queue v0.3</h1>
  <div class="muted">生成时间：${esc(report.generatedAt)} · 本页面只读 · 草稿保存在浏览器 localStorage · 可导出 JSON</div>
</header>
...
<main>
  ...
  <section class="quick">
    <button data-priority="P1">只看 P1</button>
    <button data-priority="P2">只看 P2</button>
    <button data-priority="P3">只看 P3</button>
    <button data-source="identity">只看 identity</button>
    <button data-source="work_graph">只看 work_graph</button>
    <button id="onlyDrafts">只看已记录草稿</button>
    <button id="exportDrafts">导出草稿 JSON</button>
    <button id="clearDrafts">清空本地草稿</button>
  </section>
  ...
  <section class="table-wrap">
    <table>
      <thead>
        <tr>
          <th>Priority</th><th>Source</th><th>Issue</th><th>Title</th><th>Source ID</th>
          <th>Candidate</th><th>Score</th><th>Review Draft</th><th>Suggested Action</th><th>Reason</th><th>Safety</th>
        </tr>
      </thead>
...
const el = {
  ...
  onlyDrafts: document.querySelector('#onlyDrafts'),
  exportDrafts: document.querySelector('#exportDrafts'),
  clearDrafts: document.querySelector('#clearDrafts'),
...
function rowKey(row) {
  return row.combinedQueueId;
}

function rowText(row) {
  return [
    row.priority, row.queueSource, row.issueType, row.title, row.normalizedTitle,
    row.sourceName, row.sourceId, row.sourceRecordKey, row.candidateClass,
    row.candidateType, row.reviewReason, row.suggestedAction,
  ].join(' ').toLowerCase();
}

function isTitleMatchSample(row) {
  return row.issueType === 'identity_title_match_sample' || row.candidateType === 'title_match';
}
...
function filteredRows() {
  ...
  return rows.filter((row) => {
    ...
    if (state.onlyDrafts && !drafts[rowKey(row)]) return false;
    if (q && !rowText(row).includes(q)) return false;
    return true;
  });
}

function draftSelectHtml(row) {
  const draft = drafts[rowKey(row)] || {};
  const action = draft.reviewDecision || '';
  const options = [
    ['', '未选择'],
    ['keep_separate', '保持分开'],
    ['same_identity_candidate', '疑似同一身份/作品'],
    ['not_same_identity', '不是同一身份/作品'],
    ['defer', '暂缓'],
  ];

  return '<select data-draft-action="' + escapeHtml(rowKey(row)) + '">' +
    options.map(([value, label]) => '<option value="' + escapeHtml(value) + '"' + (action === value ? ' selected' : '') + '>' + escapeHtml(label) + '</option>').join('') +
    '</select>';
}

function draftNoteHtml(row) {
  const draft = drafts[rowKey(row)] || {};
  return '<textarea data-draft-note="' + escapeHtml(rowKey(row)) + '" placeholder="本地备注，不写入数据库">' + escapeHtml(draft.note || '') + '</textarea>';
}
...
function setDraft(key, patch) {
  ...
  if (!next.reviewDecision && !next.note) {
    delete drafts[key];
  } else {
    drafts[key] = next;
  }
...
function exportDrafts() {
  const payload = {
    ...
    version: 'local-review-drafts-v0.1',
    rows: rows.filter((row) => drafts[rowKey(row)]).map(draftPayload),
    safety: {
      localDraftOnly: true,
      payloadWrite: false,
      postgresqlWrite: false,
  ...
  a.href = url;
  a.download = 'local-review-drafts-v01.json';
  a.click();
...
function render() {
  ...
  el.onlyDrafts.classList.toggle('active', state.onlyDrafts);
  ...
  el.draftRows.textContent = Object.keys(drafts).length;
  ...
  el.resultNote.textContent = filtered.length > page.length
    ? '当前匹配 ' + filtered.length + ' 行，仅显示前 ' + page.length + ' 行。'
  ...
  el.tbody.innerHTML = page.map((row, index) => {
    ...
    const savedClass = drafts[key] ? ' saved' : '';

    return \`
      <tr data-priority="\${escapeHtml(row.priority)}" class="\${savedClass}">
        ...
        <td class="muted">\${escapeHtml(row.reviewReason)}</td>
        <td><span class="chip">\${escapeHtml(safety)}</span></td>
      </tr>
    \`;
  }).join('');

  document.querySelectorAll('[data-draft-action]').forEach((select) => {
    select.addEventListener('change', () => {
      const key = select.dataset.draftAction;
      setDraft(key, { reviewDecision: select.value });
    });
  });

  document.querySelectorAll('[data-draft-note]').forEach((textarea) => {
    textarea.addEventListener('change', () => {
      const key = textarea.dataset.draftNote;
      setDraft(key, { note: textarea.value });
    });
  });
...
el.priority.addEventListener('change', () => { state.priority = el.priority.value; render(); });
el.queueSource.addEventListener('change', () => { state.queueSource = el.queueSource.value; render(); });
el.issueType.addEventListener('change', () => { state.issueType = el.issueType.value; render(); });
el.pageSize.addEventListener('change', () => { state.pageSize = Number(el.pageSize.value); render(); });
el.hideTitleSamples.addEventListener('change', () => { state.hideTitleSamples = el.hideTitleSamples.checked; render(); });
...
document.querySelectorAll('[data-source]').forEach((button) => {
  button.addEventListener('click', () => {
    state.queueSource = state.queueSource === button.dataset.source ? '' : button.dataset.source;
    render();
  });
});

el.onlyDrafts.addEventListener('click', () => {
  state.onlyDrafts = !state.onlyDrafts;
  render();
});

el.exportDrafts.addEventListener('click', exportDrafts);

el.clearDrafts.addEventListener('click', () => {
  if (!confirm('确认清空浏览器里的本地复核草稿？不会影响仓库和数据库。')) return;
  drafts = {};
  saveDrafts();
  render();
});
...
async function main() {
  ...
  const report = {
    generatedAt: new Date().toISOString(),
    version: 'local-review-page-v0.3',
