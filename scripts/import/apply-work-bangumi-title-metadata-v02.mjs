...
const VERSION = 'work-bangumi-title-metadata-apply-v0.2'
...
const CONFIRM_TOKEN = 'apply-bangumi-metadata-v02'
...
function normKey(value) { return clean(value).toLowerCase() }
...
function localizedTitleKey(row) {
  return [row?.title, row?.language, row?.kind, row?.source].map(normKey).join('|')
}
...
function visibleTitleKeys(doc) {
  return new Set(uniqueStrings([
    doc.title,
    doc.originalTitle,
    currentLocalizedTitles(doc).map((row) => typeof row === 'string' ? row : row?.title),
    currentAliases(doc).map((row) => typeof row === 'string' ? row : row?.value),
  ]).map(normKey))
}

function mergeLocalizedTitles(doc, additions) {
  const existing = currentLocalizedTitles(doc)
  const seen = new Set(existing.map(localizedTitleKey))
  const titleKeys = visibleTitleKeys(doc)
  const next = [...existing]
  let added = 0
  for (const row of Array.isArray(additions) ? additions : []) {
    ...
    const rowKey = localizedTitleKey(candidate)
    if (seen.has(rowKey) || titleKeys.has(titleKey)) continue
    seen.add(rowKey)
...
function mergeAliases(doc, additions) {
  const existing = currentAliases(doc)
  const seen = visibleTitleKeys(doc)
  const next = [...existing]
  let added = 0
  for (const row of Array.isArray(additions) ? additions : []) {
...
function mergeSourceLinks(doc, additions) {
  ...
  const seen = new Set(existing.map((row) => normKey(row?.url || row)).filter(Boolean))
  const next = [...existing]
  let added = 0
  for (const row of Array.isArray(additions) ? additions : []) {
...
function mergeCandidateSources(doc, additions) {
  ...
  const seen = new Set(existing.map((row) => [row?.source, row?.externalId, row?.url].map(normKey).join('|')))
  const next = [...existing]
  let added = 0
  for (const row of Array.isArray(additions) ? additions : []) {
    ...
    const url = clean(row?.url)
    const key = [source, externalId, url].map(normKey).join('|')
...
function buildPatch(doc, plan, options) {
  ...
  if (localized.added > 0) { patch.localizedTitles = localized.value; stats.localizedTitles = localized.added }

  const aliases = mergeAliases(doc, additions.aliases)
  if (aliases.added > 0) { patch.aliases = aliases.value; stats.aliases = aliases.added }

  const sourceLinks = mergeSourceLinks(doc, additions.sourceLinks)
  if (sourceLinks.added > 0) { patch.sourceLinks = sourceLinks.value; stats.sourceLinks = sourceLinks.added }

  const candidateSources = mergeCandidateSources(doc, additions.candidateSources)
  if (candidateSources.added > 0) { patch.candidateSources = candidateSources.value; stats.candidateSources = candidateSources.added }

  if (additions.summary?.summary) {
    const targetPlain = normalizePlain(additions.summary.summary)
    const currentPlain = normalizePlain(richTextPlainText(doc.summary))
    if ((options.summaryMode === 'replace' || !currentPlain) && currentPlain !== targetPlain) {
      patch.summary = plainTextToLexical(additions.summary.summary)
      stats.summary = 1
    }
  }

  if (additions.date?.firstPublishedAt) {
    const targetDate = normalizeDate(additions.date.firstPublishedAt)
    const targetPrecision = clean(additions.date.firstPublishedPrecision || 'unknown')
    const targetLabel = clean(additions.date.firstPublishedLabel || additions.date.rawDate || targetDate)
    const currentDate = normalizeDate(doc.firstPublishedAt)
    const currentPrecision = clean(doc.firstPublishedPrecision)
    const currentLabel = clean(doc.firstPublishedLabel)
    if (options.dateMode === 'replace' || !currentDate) {
      if (currentDate !== targetDate) patch.firstPublishedAt = targetDate
      if (currentPrecision !== targetPrecision) patch.firstPublishedPrecision = targetPrecision
      if (currentLabel !== targetLabel) patch.firstPublishedLabel = targetLabel
      if ('firstPublishedAt' in patch || 'firstPublishedPrecision' in patch || 'firstPublishedLabel' in patch) stats.firstPublishedAt = 1
    }
  }
...
async function main() {
  ...
  const matchMode = String(args['match-mode'] || 'title_exact')
  ...
  let plans = readJsonl(inputFile).filter((plan) => matchMode === 'all' || plan.matchMode === matchMode)
  ...
  for (const plan of plans) {
    const row = { id: plan.id, slug: plan.slug, title: plan.title, matchMode: plan.matchMode, status: 'unknown', changedFields: [], stats: {}, blockers: [] }
    try {
      ...
      if (blockers.length) { row.status = 'blocked'; blocked += 1 }
      else if (Object.keys(patch).length === 0) { row.status = 'already_current'; alreadyCurrent += 1 }
      else if (apply) {
  ...
  const outputs = {
    rows: path.join(outDir, 'work-bangumi-title-metadata-apply-v02.rows.jsonl'),
    summary: path.join(outDir, 'work-bangumi-title-metadata-apply-v02-summary.json'),
    json: path.join(outDir, 'work-bangumi-title-metadata-apply-v02.json'),
  }
  const summary = {
