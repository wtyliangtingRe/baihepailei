...
const DEFAULT_PLAN = 'data_local/staging/import/payload-work-draft-plan-v01.rows.jsonl'
...
function collectDuplicates(rows, getKey) {
  ...
  return [...buckets.entries()]
    ...
    .map(([key, values]) => ({
      key,
      count: values.length,
      samples: values.slice(0, 20).map((row) => row.planId),
    }))
}
...
function hasSafeSlug(value) {
  return /^[\p{L}\p{N}][\p{L}\p{N}-]{1,198}[\p{L}\p{N}]$/u.test(val(value))
}
...
function rowIssues(row) {
  ...
  if (row.version !== 'payload-work-draft-plan-v0.1') issues.push('unexpected_version')
  ...
  if (!hasText(payload.importBatch)) issues.push('missing_import_batch')
...
function markdown(summary, samples) {
  return [
    '# Work Draft Plan Audit v0.1',
    '',
    'Read-only audit for local work draft plan rows.',
    '',
    '## Safety',
    '',
    '- No Payload access.',
...
async function main() {
  ...
  const summary = {
    ...
    version: 'work-draft-plan-audit-v0.1',
    ...
    outputs: {
      json: path.join(outDir, 'work-draft-plan-audit-v01.json'),
      summary: path.join(outDir, 'work-draft-plan-audit-v01-summary.json'),
      md: path.join(outDir, 'work-draft-plan-audit-v01.md'),
    },
    safety: {
      readOnly: true,
      payloadRead: false,
