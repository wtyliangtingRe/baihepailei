...
const VERSION = 'source-inventory-audit-v0.1'
...
async function countJsonl(file, maxSamples=3){let rows=0, parseErrors=0; const samples=[]; const rl=readline.createInterface({input:fs.createReadStream(file,'utf8'),crlfDelay:Infinity}); for await(const line of rl){const body=line.trim(); if(!body)continue; rows++; if(samples.length<maxSamples){try{samples.push(JSON.parse(body))}catch{parseErrors++}} else {try{JSON.parse(body)}catch{parseErrors++}}} return {rows,parseErrors,samples}}
function inspectJson(value){if(Array.isArray(value))return {rows:value.length,samples:value.slice(0,3),shape:'array'}; if(value&&typeof value==='object'){for(const k of ['data','items','subjects','docs','results','records']) if(Array.isArray(value[k])) return {rows:value[k].length,samples:value[k].slice(0,3),shape:`object.${k}`}; return {rows:1,samples:[value],shape:'object'}} return {rows:0,samples:[],shape:typeof value}}
...
fs.mkdirSync(outDir,{recursive:true}); fs.writeFileSync(outputs.files,files.map(x=>JSON.stringify(x)).join('\n')+(files.length?'\n':''),'utf8'); fs.writeFileSync(outputs.sources,JSON.stringify(sources,null,2),'utf8'); fs.writeFileSync(outputs.summary,JSON.stringify(summary,null,2),'utf8'); console.log(JSON.stringify({ok:true,summary,outputs},null,2))
